...
{$ENDIF}
  , frxDBSet, FireDAC.Moni.Base, FireDAC.Moni.RemoteClient, FDConnectionPlus;
...
type
  TfmLayplan = class(TForm)
    ...
    qReadLayplansKnifeNo: TIntegerField;
    ...
    Label1: TLabel;
    Label2: TLabel;
    lblParallelogramAreaText: TLabel;
    lblParallelogramArea: TLabel;
    lblKnifeAreaTxt2: TLabel;
    lblKnifeArea: TLabel;
    Label3: TLabel;
    Label4: TLabel;
    lblBoundingAreaTxt: TLabel;
    lblBoundingArea: TLabel;
    lblBoundingRatioTxt: TLabel;
    lblBoundingRatio: TLabel;
    Label6: TLabel;
    Label8: TLabel;
    lblBoundingUnusedTxt: TLabel;
    lblKnifePercentage: TLabel;
    Label9: TLabel;
    Label10: TLabel;
    Label11: TLabel;
    Label12: TLabel;
    lblKnifePerimeterTxt: TLabel;
    lblHullPerimeterTxt: TLabel;
    lblKnifePerimeter: TLabel;
    lblHullPerimeter: TLabel;
    lblBoundingDimensionTxt: TLabel;
    lblAHRatioText: TLabel;
    lblConcavityAreaTxt: TLabel;
    lblHullAreaText: TLabel;
    lblBoundingDimension: TLabel;
    lblAHRatio: TLabel;
    lblConcavityArea: TLabel;
    lblHullArea: TLabel;
    ...
    procedure odBitmapShow(Sender: TObject);
  private
    ...
    HoldWheel: Boolean;
    SmallScale: Real;     //Added later to take into account when bmp is smaller than the image
  public
    { Public declarations }
    KnifeCode, KnifeScale, KnifeSize: string;
    KnifeArea: Real;
    LoadingLayplanning, AbortLayplanning: Boolean;
...
uses AllLayplans, AllSyntheticMaterials, FILT, FastGeo,
...
type
  TLayplan = record
    ...
    ParallelogramAreaPerKnife: real;
  end;
  TSaveKnife = record
    Save: Boolean;
    NewNo: short;
...
begin
  ...
  lblParallelogramArea.Caption := '';
  lblKnifeArea.Caption := '';
  lblBoundingArea.Caption := '';
  lblBoundingRatio.Caption := '';
  lblKnifePercentage.Caption := '';
  lblKnifePerimeter.Caption := '';
  lblHullPerimeter.Caption := '';
  lblBoundingDimension.Caption := '';
  lblAHRatio.Caption := '';
  lblHullArea.Caption := '';
  lblConcavityArea.Caption := '';
...
begin
  qPatterns.ParamByName('KnifeCode').AsANSIString := Code;
  qPatterns.ParamByName('KnifeScale').AsANSIString := Scale;
  qPatterns.ParamByName('KnifeSize').AsANSIString := Size;
  qPatterns.open;
...
begin
  ...
  LoadingLayplanning := true;
  BusyPrinting := false;
  BusyOpening := false;
  BusyReading := false;

  Caption := 'Synthetic Layplanning Loading...';
...
begin
  ...
  begin
    ...
    MResult := fmAllPatterns.ShowModal;
    if MResult = mrYes then
    begin
      sedtEdge.Value := fmAllPatterns.MatEdge;
...
var
  PerimeterTmp: TPolygon2D;
  PerimeterKnife, PerimeterHull, AreaKnife, AreaHull: real;
  HeightKnife, WidthKnife: real;
  SPerimeterKnife, sPerimeterHull, sAreaKnife: string;
  sAreaHull, sAreaConc, sAHRatio: string;
  sHeightKnife, sWidthKnife: string;
  ...
  begin
    ...
    ErrorFileExists := FileExists('Layplans_Para.csv');

    AssignFile(OutFile, 'Layplans_Para.csv');
    ...
    begin
      ...
      sl.Add('Knife Scale');
      sl.Add('Knife Size');
      sl.Add('Knife Perimeter');
      sl.Add('Knife Hull Perimeter');
      sl.Add('Cut Gap');
      sl.Add('Type');
      sl.Add('Pack Code');
      sl.Add('Start Angle');
      sl.Add('Angle');
      sl.Add('Parallelogram Area');
      sl.Add('Knife Area');
      sl.Add('Bounding Area');
      sl.Add('Bounding Unused');
      sl.Add('Bounding Unused %');
      sl.Add('Bounding Ratio');

      Writeln(OutFile, sl.DelimitedText);

      sl.Clear;
    end;

    sl.Add(KnifeCode); //Knife Code
    sl.Add(KnifeScale); //Knife Scale
    sl.Add(KnifeSize); //Knife Size
    sl.Add(trim(sPerimeterKnife));
    sl.Add(trim(sPerimeterHull));
    sl.Add(IntToStr(sedtExpand.Value)); //Cut Gap
    if (rgOptimisation.ItemIndex = 0) then
      sl.Add('Speed')
    else
      sl.Add('Accuracy');
    sl.Add(trim(sgLayPlans.Cells[0, sgLayplans.Row])); //PackCode
    sl.Add(trim(lblAdjustmentAngle.Caption)); //Start Angle
    sl.Add(trim(sgLayPlans.Cells[1, sgLayplans.Row])); //Angle
    sl.Add(trim(sgLayPlans.Cells[10, sgLayplans.Row])); //Parallelogram Area
    sl.Add(trim(sgLayPlans.Cells[12, sgLayplans.Row])); //Knife Area
    sl.Add(trim(sgLayPlans.Cells[13, sgLayplans.Row])); //Bounding Area
    sl.Add(trim(sgLayPlans.Cells[15, sgLayplans.Row])); //Bounding Unused
    sl.Add(trim(sgLayPlans.Cells[16, sgLayplans.Row])); //Bounding Unused %
    sl.Add(trim(sgLayPlans.Cells[14, sgLayplans.Row])); //Bounding Ratio
...
var
  ...
  j: integer;
  tp2DPattern: TPolygon2D;
  tempPattern: TPolygon2D;
  tlPoint, brPoint: TPoint2D;
  tempSideA, tempSideB: real;
  tempArea, tempRatio: real;
  tempHull: TPolygon2D;
...
begin
  ...
  SetLength(PerimeterTmp, Length(Knife.PatternPoints));
  for i := 0 to Length(Knife.PatternPoints) - 1 do
  begin
    PerimeterTmp[i].x := Knife.PatternPoints[i].X;
    PerimeterTmp[i].y := Knife.PatternPoints[i].Y;
  end;

  tempHull := MakeConvexHull(Knife.PatternPoints);

  PerimeterKnife := Perimeter(PerimeterTmp);
  PerimeterKnife := PerimeterKnife / ((1000 div PATTERNRES) * 12);
  PerimeterKnife := PerimeterKnife / 3 * 0.9144;
  PerimeterKnife := PerimeterKnife * 100;
  str(PerimeterKnife : 5 : 2, sPerimeterKnife);

  AreaKnife := Area(PerimeterTmp);
  AreaKnife := AreaKnife / ((1000 div PATTERNRES) * (1000 div PATTERNRES) * 144);
  AreaKnife := AreaKnife / 9 * 0.8361;
  AreaKnife := AreaKnife * 10000;
  str(AreaKnife : 5 : 2, sAreaKnife);

  PerimeterHull := Perimeter(tempHull);
  PerimeterHull := PerimeterHull / ((1000 div PATTERNRES) * 12);
  PerimeterHull := PerimeterHull / 3 * 0.9144;
  PerimeterHull := PerimeterHull * 100;
  str(PerimeterHull : 5 : 2, sPerimeterHull);

  AreaHull := Area(tempHull);
  AreaHull := AreaHull / ((1000 div PATTERNRES) * (1000 div PATTERNRES) * 144);
  AreaHull := AreaHull / 9 * 0.8361;
  AreaHull := AreaHull * 10000;
  str(AreaHull : 5 : 2, sAreaHull);

  str((AreaKnife / PerimeterHull) : 5 : 2, sAHRatio);
  str((AreaHull - AreaKnife) : 5 : 2, sAreaConc);

  HeightKnife := HighestPoint(tempHull).y - LowestPoint(tempHull).y;
  HeightKnife := HeightKnife / ((1000 div PATTERNRES) * 12);
  HeightKnife := HeightKnife / 3 * 0.9144;
  HeightKnife := HeightKnife * 100;
  str(HeightKnife : 5 : 2, sHeightKnife);

  WidthKnife := MostRightPoint(tempHull).x - MostLeftPoint(tempHull).x;
  WidthKnife := WidthKnife / ((1000 div PATTERNRES) * 12);
  WidthKnife := WidthKnife / 3 * 0.9144;
  WidthKnife := WidthKnife * 100;
  str(WidthKnife : 5 : 2, sWidthKnife);


//                    Utilisation := Abs(Area(KnivesUsed[0, 0].ConvexHull)) / ((1000 div PATTERNRES) * (1000 div PATTERNRES) * 144 * ParallelogramCount);
//                    Utilisation := Utilisation / 9 * 0.8361;  //SqM
//                    Utilisation := Utilisation * 10000;
//
//                    Utilisation := Abs(Area(Parallelogram)) / ((1000 div PATTERNRES) * (1000 div PATTERNRES) * 144 * ParallelogramCount);
//                    Utilisation := Utilisation / 9 * 0.8361;  //SqM
//                    Utilisation := Utilisation * 10000;

  SetLength(PerimeterTmp, 0);
  ...
  tp2DPattern := ConvertTPointArray_TPolygon2D(Knife.PatternPoints);
  KnifeSmallestArea := MaxDouble;
  KnifeSmallestRatio := MaxDouble;
  tempArea := 0;
  tempRatio := 0;

  for j := 1 to 90 do
  begin
    tempPattern := Rotate(j, tp2DPattern);

    tlPoint.x := MostLeftPoint(tempPattern).x;
    tlPoint.y := HighestPoint(tempPattern).y;
    brPoint.x := MostRightPoint(tempPattern).x;
    brPoint.y := LowestPoint(tempPattern).y;

    tempSideA := brPoint.x - tlPoint.x;
    tempSideB := tlPoint.y - brPoint.y;

    tempArea := tempSideA * tempSideB;
    tempRatio := tempSideA / tempSideB;

    if tempRatio < 1 then
      tempRatio := 1 / tempRatio;

    if tempArea < KnifeSmallestArea then
    begin
      KnifeSmallestArea := tempArea;
      KnifeSmallestRatio := tempRatio;
    end;
  end;

  KnifeSmallestArea := Abs(KnifeSmallestArea) / ((1000 div PATTERNRES) * (1000 div PATTERNRES) * 144);
...
//                      SetLength(Layplans[length(LayPlans) - 1].Parallelogram, Length(Parallelogram));
//                      for j := 0 to Length(Parallelogram) - 1 do
//                      begin
//                        Layplans[length(LayPlans) - 1].Parallelogram[j].x := Parallelogram[j].x;
//                        Layplans[length(LayPlans) - 1].Parallelogram[j].y := Parallelogram[j].y;
//                      end;
                      Layplans[length(LayPlans) - 1].Parallelogram := Parallelogram;
                      Layplans[length(LayPlans) - 1].ParallelogramAreaPerKnife := Utilisation;
                    end;
                    application.processmessages;
                    ...
                    Layplans[length(LayPlans) - 1].ParallelogramAreaPerKnife := Utilisation;
                    str(Utilisation : 5 : 2, s);
                    sgLayPlans.Cells[10, sgLayPlans.RowCount - 1] := s;
                    str(Utilisation + 1000000 : 5 : 2, s);
                    sgLayPlans.Cells[11, sgLayPlans.RowCount - 1] := s;

                    Utilisation := KnivesUsed[0, 0].PatternNettArea;
                    Utilisation := Utilisation / 9 * 0.8361;  //SqM
                    Utilisation := Utilisation * 10000;
                    str(Utilisation : 5 : 2, s);
                    sgLayPlans.Cells[12, sgLayPlans.RowCount - 1] := s;

                    Utilisation := KnifeSmallestArea;
                    Utilisation := Utilisation / 9 * 0.8361;  //SqM
                    Utilisation := Utilisation * 10000;
                    str(Utilisation : 5 : 2, s);
                    sgLayPlans.Cells[13, sgLayPlans.RowCount - 1] := s;

                    Utilisation := KnifeSmallestRatio;
                    str(Utilisation : 5 : 2, s);
                    sgLayPlans.Cells[14, sgLayPlans.RowCount - 1] := s;

                    Utilisation := KnifeSmallestArea - KnivesUsed[0, 0].PatternNettArea;
                    Utilisation := Utilisation / 9 * 0.8361;  //SqM
                    Utilisation := Utilisation * 10000;
                    str(Utilisation : 5 : 2, s);
                    sgLayPlans.Cells[15, sgLayPlans.RowCount - 1] := s;

                    Utilisation := (KnifeSmallestArea - KnivesUsed[0, 0].PatternNettArea) / KnifeSmallestArea;
                    Utilisation := Utilisation * 100;
                    str(Utilisation : 5 : 2, s);
                    sgLayPlans.Cells[16, sgLayPlans.RowCount - 1] := s;
...
//                        writeln(OutFile, s + ' Utilisation Warning (>90%)');
  ...
  Calculating := False;
  LayPlansLoaded := (PlanNo <> 0) and not AbortLayplanning;
  ...
  begin
//    SortResults;
    SortStringGrid(sgLayPlans, 11, true);
...
//
//    if sgLayPlans.RowCount >= 1 then
//    begin
//      sgLayPlans.Row := 0;
//      sgLayPlansDblClick(sgLayPlans);
//    end;
//  end;
  ...
  begin
    ...
    sgLayPlansDblClick(sgLayPlans);
    SaveToFile();
    lblKnifePerimeter.Caption := sPerimeterKnife + ' cm';
    lblHullPerimeter.Caption := sPerimeterHull + ' cm';
    lblBoundingDimension.Caption := 'H:' + sHeightKnife + ' x W:' + sWidthKnife + ' cm';
    lblAHRatio.Caption := sAHRatio + ' cm';
    lblHullArea.Caption := sAreaHull + ' cm';
    lblConcavityArea.Caption := sAreaConc + ' cm';
  end;
  ...
  begin
    ...
    MessageDlgPos('No Layplans were created', mtInformation, [mbOk], 0, ((Application.MainForm.Width div 3) + Application.MainForm.Left), ((Application.MainForm.Height div 3) + Application.MainForm.Top));
...
{$IFDEF DEBUG}
  ...
  if (WholeCreate <> 0) and
     (TotalInFindInterlocks <> 0) and
     (TotalFullPlan <> 0) and
     (TotalUpdateSynthRes <> 0) then
    MessageDlgPos('KnifeCode - ' + KnifeCode + #13 + #13 +
               'Pattern - ' + #13 +
               ...
               'Merge Patterns: ' + IntToStr(round(TotalDoMerge / TotalUpdateSynthRes * 100)) + '%', mtInformation, [mbOK], 0, ((Application.MainForm.Width div 3) + Application.MainForm.Left), ((Application.MainForm.Height div 3) + Application.MainForm.Top));
...
var
  ...
  x1, y1, x2, y2: Integer;
  UnZoomed_Display_VisibleMaterialLength: integer;
...
//  if Option_ProductionSystem then //Now available in Costings Only version too
  begin
    ...
    UnZoomed_Display_VisibleMaterialLength := Display_VisibleMaterialLength;
    Display_VisibleMaterialLength := Display_VisibleMaterialLength * Zoom;
    if not (IsRoll and mnuLayPlansShowWholeLength.Checked) then
      if Display_VisibleMaterialLength > Display_MaterialWidth then
        Display_VisibleMaterialLength := Display_MaterialWidth;
    ...
    yOffset := yOffset + Display_VisibleMaterialLength - UnZoomed_Display_VisibleMaterialLength;
...
begin
  ...
  begin
    ...
    begin
      ...
      if not FromBulkLayplanning then
        MessageDlgPos('Whole Length viewable only up to ' + intToStr(ROLL_LENGTH_WIDTH_RATIO) + ' x Width', mtInformation, [mbOk], 0, ((Application.MainForm.Width div 3) + Application.MainForm.Left), ((Application.MainForm.Height div 3) + Application.MainForm.Top));
    end;
  ...
  //Override if can still see top
  if rulerLayPlanLeft.StartValue = 0 then
    imgJaggyEdgeTop.Visible := False;
...
begin
  if LayPlansLoaded and not AbortLayPlanning then
  begin
    DisplayPlan := True;
...
//                        Layplans[length(LayPlans) - 1].Parallelogram[j].y := Parallelogram[j].y;
//                      end;
    ResultsParallelogram := Layplans[length(LayPlans) - 1].Parallelogram;
    ResultsParallelogramAreaForKnife := Layplans[length(LayPlans) - 1].ParallelogramAreaPerKnife;
...
begin
  ...
  begin
    ...
    //Check for Zoomed materials which are wider than their length
    if MaterialLength < MaterialWidth then
    begin
      if ActualMaterialLength > ((rulerLayPlanLeft.Height / rulerLayPlanLeft.UnitSize) * 10) then
        rulerLayPlanLeft.StartValue := (ActualMaterialLength - ((rulerLayPlanLeft.Height / rulerLayPlanLeft.UnitSize) * 10)) / 10
      else
        rulerLayPlanLeft.StartValue := 0;
    end;
...
procedure TfmLayplan.odBitmapShow(Sender: TObject);
var
  dlgRect: TRect;
...
begin
  if BusyPrinting or BusyOpening or BusyReading or LoadingLayplanning or (Pos('Loading...', Caption) > 0) then
  begin
  ...
  begin
    ...
    {$IFNDEF STANDALONE}
      if Assigned(fmAllPatterns) then fmAllPatterns.Release;
      if Assigned(fmAllLayplans) then fmAllLayplans.Release;
      if Assigned(fmAllLayplansWithoutSelection) then fmAllLayplansWithoutSelection.Release;
      if Assigned(fmAllSyntheticMaterials) then fmAllSyntheticMaterials.Release;
    {$IFDEF DEBUGFULL}
      if Assigned(fmDebugger) then fmDebugger.Release;
    {$ENDIF}
...
begin
  ...
  if (not CanClose) then
    MessageDlgPos('Can not close whilst calculating or saving', mtInformation, [mbOk], 0, ((Application.MainForm.Width div 3) + Application.MainForm.Left), ((Application.MainForm.Height div 3) + Application.MainForm.Top));

{$IFNDEF STANDALONE}
  if CanClose and (CanSave or SelectedLayplanChanged) and (not fmSumms.ClosingSystem) and (not FromBulkLayplanning) then
    CanClose := (MessageDlgPos('Close without saving?', mtConfirmation, [mbYes, mbNo], 0, ((Application.MainForm.Width div 3) + Application.MainForm.Left), ((Application.MainForm.Height div 3) + Application.MainForm.Top)) = mrYes);
{$ENDIF}
end;
...
begin
  ...
  begin
    if not FromBulkLayplanning then
      MessageDlgPos('Knife ' + Code + ', Size ' + Size + ' not found.', mtInformation, [mbOk], 0, ((Application.MainForm.Width div 3) + Application.MainForm.Left), ((Application.MainForm.Height div 3) + Application.MainForm.Top));
  end
  else
  begin
...
//                           '(MaterialCodeRestrictive ' + sReference1 + ');' + #13;
  ...
  SQLStrings.Add('DELETE FROM LayplanSets ' +
                           'WHERE (KnifeCode = ''' + QS(KnifeCode) + ''') AND ' +
                           '(KnifeSizeScale = ''' + QS(KnifeScale) + ''') AND ' +
                           '(KnifeSize = ''' + QS(KnifeSize) + ''') AND ' +
                           '(MaterialLength = ' + sMaterialLength + ') AND ' +
  ...
  SQLStrings.Add('DELETE FROM LayplanSetKnives ' +
                           'WHERE (KnifeCode = ''' + QS(KnifeCode) + ''') AND ' +
                           '(KnifeSizeScale = ''' + QS(KnifeScale) + ''') AND ' +
                           '(KnifeSize = ''' + QS(KnifeSize) + ''') AND ' +
                           '(MaterialLength = ' + sMaterialLength + ') AND ' +
  ...
  SQLStrings.Add('DELETE FROM LayplanPlans ' +
                           'WHERE (KnifeCode = ''' + QS(KnifeCode) + ''') AND ' +
                           '(KnifeSizeScale = ''' + QS(KnifeScale) + ''') AND ' +
                           '(KnifeSize = ''' + QS(KnifeSize) + ''') AND ' +
                           '(MaterialLength = ' + sMaterialLength + ') AND ' +
  ...
  SQLStrings.Add('DELETE FROM LayplanPlanPacks ' +
                           'WHERE (KnifeCode = ''' + QS(KnifeCode) + ''') AND ' +
                           '(KnifeSizeScale = ''' + QS(KnifeScale) + ''') AND ' +
                           '(KnifeSize = ''' + QS(KnifeSize) + ''') AND ' +
                           '(MaterialLength = ' + sMaterialLength + ') AND ' +
...
//                                   sSelected + '); ' + #13;
  ...
  SQLStrings.Add('INSERT INTO LayplanSets (KnifeCode, ' +
                                                    'KnifeSizeScale, ' +
                                                    'KnifeSize, ' +
                                                    'MaterialLength, ' +
                                                    'MaterialWidth, ' +
...
//                              sY + '); ' + #13;
          ...
          SQLStrings.Add('INSERT INTO LayplanSetKnives (KnifeCode, ' +
                                                                 'KnifeSizeScale, ' +
                                                                 'KnifeSize, ' +
                                                                 'MaterialLength, ' +
                                                                 'MaterialWidth, ' +
...
//                          sKnifeNo + '); ' + #13;
      ...
      SQLStrings.Add('INSERT INTO LayplanPlans (KnifeCode, ' +
                                                         'KnifeSizeScale, ' +
                                                         'KnifeSize, ' +
                                                         'MaterialLength, ' +
                                                         'MaterialWidth, ' +
...
//                            sBottom + '); ' + #13;
        ...
        SQLStrings.Add('INSERT INTO LayplanPlanPacks (KnifeCode, ' +
                                                               'KnifeSizeScale, ' +
                                                               'KnifeSize, ' +
                                                               'MaterialLength, ' +
                                                               'MaterialWidth, ' +
...
//  qSaveLayplan.SQL := SQLStrings;
  LocalConnectionSumms.StartTransaction;
  try
    k := 0;
    while k < SQLStrings.Count do
    begin
      qSaveLayplan.SQL.Text := SQLStrings.Strings[k];
...
//    qSaveLayplan.ExecSQL;
    ...
    begin
      ...
      begin
        ...
        if (pos('Cannot modify a read-only dataset', (E as EDataBaseError).Message) > 0) or
           (pos('permissions have not been granted to the user', (E as EDataBaseError).Message) > 0) then
          ADMessage('Rights are not granted for User - ' + SystemUserName)
        else
          fmErrorHandler.DbErrMsg(E);
      end
      else if E is EFDDBEngineException then
...
begin
  qDeleteLayplan.ParamByName('KnifeCode').AsANSIString := KnifeCode;
  qDeleteLayplan.ParamByName('KnifeSizeScale').AsANSIString := KnifeScale;
  qDeleteLayplan.ParamByName('KnifeSize').AsANSIString := KnifeSize;
  qDeleteLayplan.ParamByName('MaterialLength').AsInteger := MaterialLength;
  qDeleteLayplan.ParamByName('MaterialWidth').AsInteger := MaterialWidth;
  qDeleteLayplan.ParamByName('MaterialCutGap').AsInteger := sedtExpand.Value;
  if pedtRestrictiveMaterial.Text = '' then
    qDeleteLayPlan.ParamByName('MaterialCodeRestrictive').Clear
  else
    qDeleteLayPlan.ParamByName('MaterialCodeRestrictive').AsANSIString := pedtRestrictiveMaterial.Text;
...
begin
  if MessageDlgPos('Delete saved Layplan Set?', mtConfirmation, [mbYes, mbNo], 0, ((Application.MainForm.Width div 3) + Application.MainForm.Left), ((Application.MainForm.Height div 3) + Application.MainForm.Top)) = mrYes then
  begin
    DeleteLayplanSet;
    ClearLayplans;
  end;
...
//  begin
    ...
    frLayplan.PrintOptions.ShowDialog := not GroupPrinting;
    ...
    begin
      ...
      PrintingCancelled := not frLayplan.Print;
//  end
  except
{$IFNDEF STANDALONE}
...
begin
  ...
  begin
    SelectedLayplan := -1;
    lblSelectedLayplan.Caption := '<None>';
    lblParallelogramArea.Caption := '<None>';
    lblKnifeArea.Caption := '<None>';
    lblBoundingArea.Caption := '<None>';
    lblBoundingRatio.Caption := '<None>';
    lblKnifePercentage.Caption := '<None>';
    lblKnifePerimeter.Caption := '<None>';
    lblHullPerimeter.Caption := '<None>';
    lblBoundingDimension.Caption := '<None>';
    lblAHRatio.Caption := '<None>';
    lblHullArea.Caption := '<None>';
    lblConcavityArea.Caption := '<None>';
  end
  else
  begin
    SelectedLayplan := sgLayplans.Row;
    lblSelectedLayplan.Caption := sgLayplans.Cells[0, sgLayplans.Row];
    lblParallelogramArea.Caption := sgLayplans.Cells[10, sgLayplans.Row] + ' Sq cm';
    lblKnifeArea.Caption := sgLayplans.Cells[12, sgLayplans.Row] + ' Sq cm';
    lblBoundingArea.Caption := sgLayplans.Cells[13, sgLayplans.Row] + ' Sq cm';
    lblBoundingRatio.Caption := sgLayplans.Cells[14, sgLayplans.Row];
    lblKnifePercentage.Caption :=  sgLayplans.Cells[15, sgLayplans.Row] + ' Sq cm (' + sgLayplans.Cells[16, sgLayplans.Row] + '%)'
  end;
...
var
  sMaterialLength, sMaterialWidth, sCutGap, sEdge, sReference1, SQLString: string;
  Exists: Boolean;
...
begin
  ...
  if pedtRestrictiveMaterial.Text = '' then
    ...
    sReference1 := '= ''' + Trim(QS(pedtRestrictiveMaterial.text)) + '''';
  ...
  SQLString := SQLString + 'SELECT * FROM LayplanSets ' +
                           ...
                           '(MaterialWidth = ' + sMaterialWidth + ') AND ' +
                           '(MaterialCutGap = ' + sCutGap + ') AND ' +
                           '(MaterialCodeRestrictive ' + sReference1 + ') AND ' +
                           '(MaterialEdge = ' + sEdge + ')' + #13;
  qSaveLayplan.SQL.Text := SQLString;
  qSaveLayplan.open;
...
begin
  ...
  //Layplan Set
  qReadLayplanSets.ParamByName('KnifeCode').AsANSIString := KnifeCode;
  qReadLayplanSets.ParamByName('KnifeSizeScale').AsANSIString := KnifeScale;
  qReadLayplanSets.ParamByName('KnifeSize').AsANSIString := KnifeSize;
  qReadLayplanSets.ParamByName('MaterialLength').AsInteger := MaterialLength;
  qReadLayplanSets.ParamByName('MaterialWidth').AsInteger := MaterialWidth;
  qReadLayplanSets.ParamByName('MaterialCutGap').AsInteger := sedtExpand.Value;
  if pedtRestrictiveMaterial.Text = '' then
    qReadLayplanSets.ParamByName('MaterialCodeRestrictive').Clear
  else
    qReadLayplanSets.ParamByName('MaterialCodeRestrictive').AsANSIString := pedtRestrictiveMaterial.Text;
  ...
  //Knives
  qReadNumberOfLayPlanKnives.ParamByName('KnifeCode').AsANSIString := KnifeCode;
  qReadNumberOfLayPlanKnives.ParamByName('KnifeSizeScale').AsANSIString := KnifeScale;
  qReadNumberOfLayPlanKnives.ParamByName('KnifeSize').AsANSIString := KnifeSize;
  qReadNumberOfLayPlanKnives.ParamByName('MaterialLength').AsInteger := MaterialLength;
  qReadNumberOfLayPlanKnives.ParamByName('MaterialWidth').AsInteger := MaterialWidth;
  qReadNumberOfLayPlanKnives.ParamByName('MaterialCutGap').AsInteger := sedtExpand.Value;
  if pedtRestrictiveMaterial.Text = '' then
    qReadNumberOfLayPlanKnives.ParamByName('MaterialCodeRestrictive').Clear
  else
    qReadNumberOfLayPlanKnives.ParamByName('MaterialCodeRestrictive').AsANSIString := pedtRestrictiveMaterial.Text;

  qReadNumberOfLayPlanKnives.Open;
  NoKnivesToLoad := qReadNumberOfLayPlanKnivesNoKnives.value;
  qReadNumberOfLayPlanKnives.Close;

  qReadLayPlanKnives.ParamByName('KnifeCode').AsANSIString := KnifeCode;
  qReadLayPlanKnives.ParamByName('KnifeSizeScale').AsANSIString := KnifeScale;
  qReadLayPlanKnives.ParamByName('KnifeSize').AsANSIString := KnifeSize;
  qReadLayPlanKnives.ParamByName('MaterialLength').AsInteger := MaterialLength;
  qReadLayPlanKnives.ParamByName('MaterialWidth').AsInteger := MaterialWidth;
  qReadLayPlanKnives.ParamByName('MaterialCutGap').AsInteger := sedtExpand.Value;
  if pedtRestrictiveMaterial.Text = '' then
    qReadLayPlanKnives.ParamByName('MaterialCodeRestrictive').Clear
  else
    qReadLayPlanKnives.ParamByName('MaterialCodeRestrictive').AsANSIString := pedtRestrictiveMaterial.Text;
  ...
  //Layplans
  qReadLayPlans.ParamByName('KnifeCode').AsANSIString := KnifeCode;
  qReadLayPlans.ParamByName('KnifeSizeScale').AsANSIString := KnifeScale;
  qReadLayPlans.ParamByName('KnifeSize').AsANSIString := KnifeSize;
  qReadLayPlans.ParamByName('MaterialLength').AsInteger := MaterialLength;
  qReadLayPlans.ParamByName('MaterialWidth').AsInteger := MaterialWidth;
  qReadLayPlans.ParamByName('MaterialCutGap').AsInteger := sedtExpand.Value;
  if pedtRestrictiveMaterial.Text = '' then
    qReadLayPlans.ParamByName('MaterialCodeRestrictive').Clear
  else
    qReadLayPlans.ParamByName('MaterialCodeRestrictive').AsANSIString := pedtRestrictiveMaterial.Text;
  qReadLayplans.Open;

  //Layplan Pack ststic values
  qReadLayplanPacks.ParamByName('KnifeCode').AsANSIString := KnifeCode;
  qReadLayplanPacks.ParamByName('KnifeSizeScale').AsANSIString := KnifeScale;
  qReadLayplanPacks.ParamByName('KnifeSize').AsANSIString := KnifeSize;
  qReadLayplanPacks.ParamByName('MaterialLength').AsInteger := MaterialLength;
  qReadLayplanPacks.ParamByName('MaterialWidth').AsInteger := MaterialWidth;
  qReadLayplanPacks.ParamByName('MaterialCutGap').AsInteger := sedtExpand.Value;
  if pedtRestrictiveMaterial.Text = '' then
    qReadLayplanPacks.ParamByName('MaterialCodeRestrictive').Clear
  else
    qReadLayplanPacks.ParamByName('MaterialCodeRestrictive').AsANSIString := pedtRestrictiveMaterial.Text;
  qReadLayplanPacks.Open;
  qReadLayplanPacks.Filtered := true;
...
//    qReadLayplanPacks.ParamByName('LayplanCode').AsANSIString := LayplanCode;
//    qReadLayplanPacks.Open;
    qReadLayplanPacks.Filter := '(LayplanCode = '''+ LayplanCode +''')';
    qReadLayplanPacks.Filtered := true;

    qReadLayplanPacks.RecNo := 1; //CJY changed from qReadLayplans.First
    qReadLayplanPacks.Prior; //CJY changed from qReadLayplans.First
...
//    qReadLayplanPacks.Close;
  ...
  qReadLayplanPacks.Filtered := false;
  qReadLayplanPacks.Close;
  qReadLayplans.Close;
...
var
  ...
  sMaterialLength, sMaterialWidth, sCutGap, sReference1, sEdge: string;
  Pieces: integer;
...
begin
  ...
  if FromBulkLayplanning or (not Option_ProductionSystem) then
    SaveLayplans := True
  else if MessageDlgPos(s, mtConfirmation, [mbYes, mbNo], 0, ((Application.MainForm.Width div 3) + Application.MainForm.Left), ((Application.MainForm.Height div 3) + Application.MainForm.Top)) = mrYes then
  begin
    if CanSave then
      SaveLayplans := True
    else if SelectedLayplanChanged then
  ...
  begin
    ...
    begin
      ...
      begin
        if MessageDlgPos('Layplans already exist. Saving new layplans over' + #13 +
                      'current ones will affect existing allowances.' + #13#13 +
                      'Save new Layplans?',
                      mtConfirmation, [mbYes, mbNo], 0, ((Application.MainForm.Width div 3) + Application.MainForm.Left), ((Application.MainForm.Height div 3) + Application.MainForm.Top)) = mrNo then
          SaveLayplans := False;
      end
