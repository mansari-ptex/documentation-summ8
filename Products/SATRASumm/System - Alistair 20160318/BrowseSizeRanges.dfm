object fmBrowseSizeRanges: TfmBrowseSizeRanges
  Left = 199
  Top = 228
  BorderStyle = bsDialog
  Caption = 'Browse Size Ranges'
  ClientHeight = 366
  ClientWidth = 635
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = True
  Position = poScreenCenter
  OnCreate = FormCreate
  OnHide = FormHide
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object tbMain: TPanel
    Left = 0
    Top = 0
    Width = 635
    Height = 29
    Align = alTop
    Color = clAqua
    TabOrder = 0
    object btnRefresh: TSpeedButton
      Left = 3
      Top = 0
      Width = 23
      Height = 23
      Hint = 'Refresh'
      AllowAllUp = True
      Flat = True
      Glyph.Data = {
        
        3393333888333888338333333999993333333333388888333333}
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = True
      OnClick = btnRefreshClick
    end
  end
  object pnlSearch: TPanel
    Left = 0
    Top = 29
    Width = 635
    Height = 41
    Align = alTop
    BevelOuter = bvNone
    ParentColor = True
    TabOrder = 1
    object lblRange: TLabel
      Left = 247
      Top = 14
      Width = 42
      Height = 13
      Caption = 'lblRange'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clFuchsia
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      Visible = False
    end
    object lblSearch: TLabel
      Left = 3
      Top = 14
      Width = 34
      Height = 13
      Caption = 'Search'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clFuchsia
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object edSearch: TEdit
      Left = 50
      Top = 10
      Width = 180
      Height = 21
      CharCase = ecUpperCase
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      MaxLength = 20
      ParentFont = False
      TabOrder = 0
      OnChange = edSearchChange
      OnKeyPress = edSearchKeyPress
    end
  end
  object dbgSizeRanges: TDBGridPlus
    AlignWithMargins = True
    Left = 3
    Top = 73
    Width = 629
    Height = 290
    Align = alClient
    BorderStyle = bsNone
    Color = clAqua
    DataSource = dsSizeRanges
    DrawingStyle = gdsClassic
    FixedColor = clLime
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick]
    ParentFont = False
    ReadOnly = True
    TabOrder = 2
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clFuchsia
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
    OnCellClick = dbgSizeRangesCellClick
    OnDblClick = dbgSizeRangesDblClick
    OnKeyPress = dbgSizeRangesKeyPress
    OnTitleClick = dbgSizeRangesTitleClick
    Columns = <
      item
        Expanded = False
        FieldName = 'Range'
        Width = 180
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Scale'
        Width = 180
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Description'
        Width = 250
        Visible = True
      end>
  end
  object qSizeRanges: TFDQueryPlus
    Connection = fmSumms.ConnectionSumms
    SQL.Strings = (
      
        'SELECT Range,Scale,Description FROM SizeRanges WHERE Range LIKE ' +
        ':Search Order By Range, Range')
    Left = 272
    Top = 107
    ParamData = <
      item
        Name = 'Search'
        ParamType = ptInput
      end>
    object qSizeRangesRange: TStringField
      FieldName = 'Range'
    end
    object qSizeRangesScale: TStringField
      FieldName = 'Scale'
    end
    object qSizeRangesDescription: TStringField
      FieldName = 'Description'
      Size = 30
    end
  end
  object dsSizeRanges: TDataSource
    DataSet = qSizeRanges
    Left = 344
    Top = 117
  end
end
