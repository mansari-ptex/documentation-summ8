object fmBrowseSizeScales: TfmBrowseSizeScales
  Left = 310
  Top = 142
  BorderStyle = bsDialog
  Caption = 'Browse Size Scales'
  ClientHeight = 366
  ClientWidth = 454
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = True
  OnCreate = FormCreate
  OnHide = FormHide
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object tbMain: TPanel
    Left = 0
    Top = 0
    Width = 454
    Height = 29
    Align = alTop
    Color = clAqua
    TabOrder = 0
    object btnRefresh: TSpeedButton
      Left = 3
      Top = 0
      Width = 23
      Height = 23
      Hint = 'Refresh'
      Flat = True
      Glyph.Data = {
        
        3393333888333888338333333999993333333333388888333333}
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = True
      OnClick = btnRefreshClick
    end
  end
  object pnlSearch: TPanel
    Left = 0
    Top = 29
    Width = 454
    Height = 41
    Align = alTop
    BevelOuter = bvNone
    ParentColor = True
    TabOrder = 1
    object lblScale: TLabel
      Left = 337
      Top = 14
      Width = 37
      Height = 13
      Caption = 'lblScale'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clFuchsia
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      Visible = False
    end
    object lblSearch: TLabel
      Left = 3
      Top = 14
      Width = 34
      Height = 13
      Caption = 'Search'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clFuchsia
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object edSearch: TEdit
      Left = 50
      Top = 10
      Width = 180
      Height = 21
      CharCase = ecUpperCase
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      MaxLength = 20
      ParentFont = False
      TabOrder = 0
      OnChange = edSearchChange
      OnKeyPress = edSearchKeyPress
    end
  end
  object dbgSizeScales: TDBGridPlus
    AlignWithMargins = True
    Left = 3
    Top = 73
    Width = 448
    Height = 290
    Align = alClient
    BiDiMode = bdLeftToRight
    BorderStyle = bsNone
    Color = clAqua
    DataSource = dsSizeScales
    DrawingStyle = gdsClassic
    FixedColor = clLime
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick]
    ParentBiDiMode = False
    ParentFont = False
    ReadOnly = True
    TabOrder = 2
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clFuchsia
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
    OnCellClick = dbgSizeScalesCellClick
    OnDblClick = dbgSizeScalesDblClick
    OnKeyPress = dbgSizeScalesKeyPress
    OnTitleClick = dbgSizeScalesTitleClick
    Columns = <
      item
        Expanded = False
        FieldName = 'Scale'
        Width = 180
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Description'
        Width = 250
        Visible = True
      end>
  end
  object qSizeScales: TFDQueryPlus
    Connection = fmSumms.ConnectionSumms
    SQL.Strings = (
      
        'SELECT Scale,Description FROM SizeScales WHERE Scale LIKE :Searc' +
        'h Order By Scale, Scale')
    Left = 147
    Top = 61
    ParamData = <
      item
        Name = 'Search'
      end>
    object qSizeScalesScale: TStringField
      FieldName = 'Scale'
    end
    object qSizeScalesDescription: TStringField
      FieldName = 'Description'
      Size = 30
    end
  end
  object dsSizeScales: TDataSource
    DataSet = qSizeScales
    Left = 155
    Top = 68
  end
end
