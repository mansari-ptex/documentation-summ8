...
uses
  Classes, Controls, Forms, Db,   FireDAC.Stan.Intf, FireDAC.Stan.Option, FireDAC.Stan.Param,FireDAC.Stan.Error, FireDAC.DatS, FireDAC.Phys.Intf, FireDAC.DApt.Intf,FireDAC.Stan.Async, FireDAC.DApt, FireDAC.Comp.DataSet, FireDAC.Comp.Client, FDQueryPlus, FDTablePlus,  BasicAlw, CmnTypes,
...
type
  TdmTicketsCreate = class(TDataModule)
...
var
  dmTicketsCreate: TdmTicketsCreate;
...
uses
  Windows, SysUtils, Summs, Times_Summs8, TicketsSplit, AdjFact, SummsVars,
  General, Dialogs, TicketsBreakdown, Const_Interlocking;

function TdmTicketsCreate.MakeTickets(WeekNo, SequenceNo: string; FromGroup: boolean): boolean;
type Ticket = record
  ...
  begin
    ...
    begin
      ...
      SQLString := 'INSERT INTO TicketTickets ' +
                   ...
                   'MaterialSkinSize, MaterialSkinTrimmed, MaterialUnits, ' +
                   ...
                   'M.CutType, M.Length, M.Width, M.SkinSize, M.Trimmed, M.Units, M.Layers, ' + AdjStr +
    ...
    begin
      ...
      repeat
        ...
        begin
          ...
          begin
            dmTicketsSplit.qGetSizePairs.SQL.Clear;
            dmTicketsSplit.qGovernor.SQL.Clear;
          ...
          begin
            ...
            begin
              ...
              dmTicketsSplit.qGovernor.SQL.Add(GovString);
            end;

            dmTicketsSplit.qGetSizePairs.SQL.Add(SQLString);

            dec(TicketNo);

            if SplittingScheme = SPLITTINGCONSECUTIVESIZES then
              dmTicketsSplit.SplitNormal(-TicketNo - 1, StrToInt(WeekNo), StrToInt(SequenceNo), Tickets[i].MaxPairs, TicketNo)
            else
              dmTicketsSplit.SplitGoverned(-TicketNo - 1, StrToInt(WeekNo), StrToInt(SequenceNo), Tickets[i].MaxPairs, TicketNo, HaveMaster);
          end;
        end;
        inc(i);
...
//        for i := 1 to qWhatTickets.RecordCount do
          ...
          begin
            ...
            begin
              ...
              sBasicAllowance := FloatToStrSQL(Allowance.BasicAllowance);
...
//Note BasicAllowance & AdjustedAllowance must remain in sq units for times.
          ...
          begin
            ...
            begin
              ...
              dmTimes_Summs8.CalculateAllTimes(TRUE, qGetData.FieldByName('StdBatchSize').value, qGetData, qTimesGridFullBatch, LoopCount, Dummy, Ints4Ticket, FALSE);
              ...
              begin
                dmTimes_Summs8.CalculateAllTimes(TRUE, SmallBatch, qGetData, qTimesGridSmallBatch, LoopCount, Dummy, Ints4Ticket, FALSE);
              ...
              begin
                ...
                SQLString := 'INSERT INTO TicketTicketsTimes ' +
                             'VALUES (' + WeekNo + ', ' + SequenceNo + ', ' + IntToStr(qGetData.FieldByName('TicketNo').AsInteger) + ', ' + IntToStr(qTimesGridFullBatch.FieldByName('Coeff').AsInteger) + ', ' + FloatToStrSQL(TotalTime) + ');';
...
function TdmTicketsCreate.AllowanceTicket: AllowanceResult;
var
...
begin
  dmBasAll.ChangeAllConnections(qBAForTickets.Connection);
  ...
  begin
    SQLString := 'SELECT PWK.Seq, PWK.Knife, SUM(PWK.Frequency) as Frequency, COUNT(PWK.WidthNo) as NumWidths, ' +
                 ...
                 '         LPS.MaterialLength = ROUND(IIF(M.Type = ''R'', ' + FloatToStrSQL(ROLLLENGTH_FT) +
                 ...
                 'K.InterlockAreaNonPrime, KS.Pieces, K.GrossArea, K.NettArea, MU.ToFeet, MU.SubUnitsPerUnit, M.SkinSize, ' +
	                'M.Length AS MatLength, M.Width AS MatWidth, M.CutType, M.Trimmed, M.LinearAllowance, SSS.Length AS SampleSizeMm, ' +
           ...
           'GROUP BY PWK.Seq, PWK.Knife, P.SampleSize, P.MadeInPairs, KS.Type, KS.CutGap, K.InterlockAreaPrimeSynthetic, ' +
                 'K.InterlockAreaNonPrime, KS.Pieces, K.GrossArea, K.NettArea, MU.ToFeet, MU.SubUnitsPerUnit, ' +
 	               'M.SkinSize, M.Length, M.Width, M.CutType, M.Trimmed, M.LinearAllowance, SSS.Length, ' +
...
procedure TdmTicketsCreate.LocalConnectionSummsBeforeConnect(
  Sender: TObject);
begin
  ConnectionSettings(Sender);
end;

procedure TdmTicketsCreate.ChangeAllConnections(Connection: TFDCustomConnection);
var
  i: integer;
...
procedure TdmTicketsCreate.RevertAllConnections();
begin
  ChangeAllConnections(LocalConnectionSumms);
end;

procedure TdmTicketsCreate.DataModuleCreate(Sender: TObject);
begin
  InUse := false;
end;
