...
begin
  ...
  begin
    ...
    if ((Sender as TSpeedButton) = btnPrintPreview) then
      frListMaterials.ShowPreparedReport
    else
  ...
  begin
    ...
    if ((Sender as TSpeedButton) = btnPrintPreview) then
      frAllMaterials.ShowPreparedReport
    else
...
begin
  ...
  if messageDlg('Delete selected Materials?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
  begin
    ...
    messageDlg('Deletion complete.' + #13#13 + 'Note: Only Materials that are not' + #13 + 'in use have been deleted.', mtInformation, [mbOK], 0);
  end;
