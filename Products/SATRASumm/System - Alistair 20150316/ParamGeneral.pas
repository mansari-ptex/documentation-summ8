...
uses
  ...
  FireDAC.Stan.Pool, FireDAC.Phys, DBGrids;
...
begin
  ...
  begin
    if messagedlg('Save Changes to Parameters?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
      btnSave.click
    else
      btnCancel.click;
  end;
...
begin
  ...
  begin
    MessageDlg('Cannot delete system units', mtInformation, [mbOK], 0);
    abort;
  end;
end;
...
begin
  if (tblMatUnitsCode.value = 'FT') or (tblMatUnitsCode.value = 'M') then
  begin
    MessageDlg('Cannot change system units', mtInformation, [mbOK], 0);
    abort;
  end;
end;
