...
begin
  ...
  if ((Sender as TSpeedButton) = btnPrintPreview) then
    frAllSuppliers.ShowPreparedReport
  else
...
begin
  ...
  if messageDlg('Delete selected Suppliers?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
  begin
    ...
    messageDlg('Deletion complete.' + #13#13 + 'Note: Only Suppliers that are not' + #13 + 'in use have been deleted.', mtInformation, [mbOK], 0);
  end;
...
begin
  ...
  begin
    dbgSuppliers.SortColumn := Column.Index;

    if dbgSuppliers.SortOrder = soAscending then
    begin
      dbgSuppliers.SortOrder := soDescending;
      SQLString := ' DESC';
    end
    else
    begin
