...
uses
  ...
  FireDAC.Stan.Def, FireDAC.Stan.Pool, FireDAC.Phys, Vcl.DBGrids;
...
begin
  AbortAssessing := True;
  MessageDlg('Assessing will abort ' + #13 +
             'after current pattern.', mtInformation, [mbOk], 0);
end;
...
begin
  ...
  if qKnives.recordCount = 0 then
    messagedlg('No patterns to assess', mtInformation, [mbOk], 0)
  else
  begin
    ...
    begin
      ...
      if ((qKnives.recordCount > 0) and (not AbortAssessing)) then
         messageDlg('Not all patterns assessed - ' + #13 +
                    'some may be locked', mtInformation, [mbOk], 0)
      //CJY: qKnives.FetchOptions.RecordCountMode set to cmTotal
      else if ((qKnives.recordCount = 0) and (not AbortAssessing)) then
      begin
        messageDlg('Pattern assessing complete', mtInformation, [mbOk], 0);
        AllowClose := True;
      end;
...
begin
  if Assessing then
  begin
    messagedlg('Cancel assessing before closing', mtInformation, [mbOk], 0);
    abort;
  end
  else
  begin
