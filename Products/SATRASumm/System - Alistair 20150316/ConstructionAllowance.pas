...
uses
  ...
  ToolWin, ActnList, frxClass, Clipbrd, frxReportPlus;
...
var
  ...
  AdjFactor: real;
  PartWidthNo: short;
  PartError, ConstructionError, NotAvailable, SecondLine: Boolean;
  Dummy: TTicketInts;
  CurrentColor: TColor;
...
//    else
  ...
  if ConstructionError then
    messagedlg('One or more Part Allowances cannot be' + #13 +
               'calculated - Open Part Allowance for reason', mtInformation, [mbOk], 0);
end;
...
begin
  if cbWidths.Items.Count = 0 then
    messagedlg('There are no common widths for this contruction', mtInformation, [mbOk], 0);
end;
...
begin
  ...
  if ((Sender as TSpeedButton) = btnPrintPreview) then
    frConstructionAllowance.ShowPreparedReport
  else
