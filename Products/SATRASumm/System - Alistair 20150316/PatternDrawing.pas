...
uses
  ...
  General_Interlocking, CmnVars, SummsVars, Forms;
...
type
  TdmPatternDrawing = class(TDataModule)
    ...
    procedure DrawVersion6Interlock(Code, Scale, Size: String;
                                    ...
                                    ImgInterlock: TImage;
                                    Knife: TPointArray);
    procedure Pass(Connection: TFDConnection);
  private
...
begin
  qPattInts.ParamByName('KnifeCode').value := Code;
  qPattInts.ParamByName('KnifeSizeScale').value := Scale;
  qPattInts.ParamByName('MeasuredSize').value := Size;
  qPattInts.open;
...
procedure TdmPatternDrawing.Pass(Connection: TFDConnection);
begin
