...
uses
  Windows, SysUtils, Summs, Times, TicketsSplit, AdjFact, SummsVars,
...
//        for i := 1 to qNoSampleKnife.RecordCount do
        ...
        if (not SyntheticFailed) and (not CreateComplete) and (not MissingSampleKnife)  then
          CreateComplete := (messagedlg('Ticket(s) Invalid.' + #13#13 + MyMessage + #13 +
                                        'Abort creation?', mtWarning, [mbYes, mbNo], 0) = mrNo);
...
//Note BasicAllowance & AdjustedAllowance must remain in sq units for times.
        ...
        messagedlg('Part: ' + Tickets[i - 1].Part + ' has no Master' + #13#13 + 'Ticket creation aborted', mtInformation, [mbOK], 0);
    end
    else if UnAssessed then
      messagedlg('Zero Interlock Area on Knife ' + #13 + Knife + #13#13 + 'Ticket creation aborted', mtInformation, [mbOK], 0)
    else if LeatherWithCutGap then
      messagedlg('Ticket is for Leather but Cut Gap on Knife ' + #13 + Knife + #13#13 + 'Ticket creation aborted', mtInformation, [mbOK], 0)
    else
      messagedlg('No Knives found.' + #13 + 'Check Construction.', mtInformation, [mbOK], 0);
