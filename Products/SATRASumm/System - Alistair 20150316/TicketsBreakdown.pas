...
uses
  ...
  FireDAC.Stan.Def, FireDAC.Stan.Pool, FireDAC.Phys, fr_Ticket, TicketsAllDM;
...
//  TmpFDConn := TFDConnection(qChangeTicketsInput.Connection);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            messagedlg('Grid NOT updated. Ticket NOT Created.', mtInformation, [mbOK], 0);
          end
          else
          if (TotalPairs > 0) then
...
begin
  ...
  begin
    ...
    begin
      if tblTicketSequences.recordcount = 0 then
      begin
        messagedlg('Ticket ' + TicketCode + ' no longer exists', mtInformation, [mbOk], 0);
        close;
      end
      else
      begin
  ...
  //            fmfrTicketLocal.qPrinted.SQL.Text := '';
              ...
              if (Sender as TControl).Name = btnPrintPreview.Name then
                fmfrTicketLocal.frTicket.ShowPreparedReport
              else
