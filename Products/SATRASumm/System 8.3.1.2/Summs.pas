...
type
  TfmSumms = class(TForm)
    ...
    imgSystem: TImage;
    qParametersSearchStyleDescriptionDefault: TBooleanField;
    qParametersLegacyCuttingTimes: TBooleanField;
    qParametersLegacyCuttingTimesExact: TBooleanField;
    qParametersLegacyCuttingTimesBase: TBooleanField;
    qParts: TFDQuery;
    ...
    qConstructions: TFDQuery;
    qConstructionsConstruction: TStringField;
    qConstructionsMadeInPairs: TBooleanField;
    qConstructionsDescription: TStringField;
    qSizeRelationships: TFDQuery;
    ...
    procedure ConnectionSummsAfterConnect(Sender: TObject);
...
uses
  ...
  BasicAlw, AdjFact, CutUtils_Summs7, CutUtils2_Summs7, Times_Summs7, Times2_Summs7,
  CutUtils_Summs8, CutUtils2_Summs8, Times_Summs8, Times2_Summs8,
...
//Routine to stop the ellusive random disco effect reported in Summs 8 and TimeLine 2
function TfmSumms.HookMainWindow(var Message: TMessage): Boolean;
const
...
begin
  ...
  begin
    ...
    Option_AllConstructionAllowancesOut := ((Feature and 2048) = 2048);
    Option_OverrideReleasedVersion := ((Feature and 4096) = 4096);  //to override ReleasedVersion flag (in LayMain)
...
begin
  ...
  LegacySortLocale := SystemIni.ReadBool('Client', 'LEGACYSORTLOCALE', False);
...
begin
  ...
  //"Decimalseparator" changes in runtime and GetFormatSettings.
  //Doubled as GetFormatSettings required in thread locale???
...
procedure TfmSumms.FormDestroy(Sender: TObject);
begin
...
begin
  ...
  SearchStyleDescriptionDefault := qParametersSearchStyleDescriptionDefault.Value;
  ...
  LegacyCuttingTimes := qParametersLegacyCuttingTimes.value;
  LegacyCuttingTimesExact := qParametersLegacyCuttingTimesExact.value;
  LegacyCuttingTimesBase := qParametersLegacyCuttingTimesBase.value;
...
//    fmSumms.caption := fmSumms.caption + '          Company : ' + Company;
  ...
  if not (Company = '') then
    sCaption := SystemName + '          Company : ' + Company
  else
    sCaption := SystemName;

  sConnection := 'Connection : ' + ConnectionSumms.Params.Values['ServerTypes'];

  if fmSumms.caption <> sCaption then
    fmSumms.caption := sCaption;

  if sbMain.Panels[1].text <> sConnection then
    sbMain.Panels[1].text := sConnection;
end;
...
procedure TfmSumms.ConnectionSummsAfterConnect(Sender: TObject);
begin
  ConnectionVersion(Sender);
...
//  tblConstructions.open;
  ...
  dmCutUtils_Summs7 := TdmCutUtils_Summs7.create(fmSumms);
  dmCutUtils2_Summs7 := TdmCutUtils2_Summs7.create(fmSumms);
  dmCutUtils_Summs8 := TdmCutUtils_Summs8.create(fmSumms);
  dmCutUtils2_Summs8 := TdmCutUtils2_Summs8.create(fmSumms);
  dmTimes_Summs7 := TdmTimes_Summs7.create(fmSumms);
  dmTimes2_Summs7 := TdmTimes2_Summs7.create(fmSumms);
  dmTimes_Summs8 := TdmTimes_Summs8.create(fmSumms);
  dmTimes2_Summs8 := TdmTimes2_Summs8.create(fmSumms);
  dmCutUtils2_Summs7.qAddCuttingElement.Connection := fmSumms.ConnectionSumms;
  dmCutUtils2_Summs8.qAddCuttingElement.Connection := fmSumms.ConnectionSumms;
