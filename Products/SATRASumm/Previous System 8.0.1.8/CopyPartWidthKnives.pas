...
begin
  ...
  if (OldWidthNo <= 0) then
    MessageDlgPos('No Width to Copy from', mtInformation, [mbOk], 0, ((Application.MainForm.Width div 3) + Application.MainForm.Left), ((Application.MainForm.Height div 3) + Application.MainForm.Top))
  else if (NewWidthNo <= 0) then
    MessageDlgPos('No Width to Copy to', mtInformation, [mbOk], 0, ((Application.MainForm.Width div 3) + Application.MainForm.Left), ((Application.MainForm.Height div 3) + Application.MainForm.Top))
  else if NewWidthNo = OldWidthNo then
    MessageDlgPos('Cannot copy to same width', mtInformation, [mbOk], 0, ((Application.MainForm.Width div 3) + Application.MainForm.Left), ((Application.MainForm.Height div 3) + Application.MainForm.Top))
  else
    GoCopy := True;

  if GoCopy then
  begin
    qKnivesOnWidth.ParamByName('Part').AsString := ParentPartCode;
    qKnivesOnWidth.ParamByName('WidthNo').AsInteger := NewWidthNo;

    qKnivesOnWidth.Open;

    if qKnivesOnWidth.FieldByName('NumKnives').Value > 0 then
      if MessageDlgPos('Overwrite knives on Width ' + cbWidthsForRange.text + '?', mtConfirmation, [mbYes, mbNo], 0, ((Application.MainForm.Width div 3) + Application.MainForm.Left), ((Application.MainForm.Height div 3) + Application.MainForm.Top)) = mrNo then
        GoCopy := False;
  ...
  begin
    ...
    try
      ...
      qCopyWidth.SQL.Text := 'DELETE FROM PtWidKnf WHERE (WidthNo = ' + sNew + ') AND (Part = ''' + QS(ParentPartCode) + ''');' + #13 +
                             ...
                             'INSERT INTO PtWidKnf (Part, WidthNo, Knife, Seq, Frequency, NoIncludedKnives, SizeScale, SizeRange, SizeRelationship, SizeAdjustment) SELECT ''' +
                             QS(ParentPartCode) + ''', ' + sNew + ', Knife, Seq, Frequency, NoIncludedKnives, SizeScale, SizeRange, SizeRelationship, SizeAdjustment ' +
