...
//  sPatternRes := IntToStr(PATTERNRES) + '.0';
...
//                        ' / 12000.0), 0) AND ' +
                        ...
                        'M.Width AS MatWidth, M.CutType, M.Trimmed, M.SheetCutFromRoll, M.AreaCoeff, M.QualCoeff, MU.ToFeet, MU.SubUnitsPerUnit, MU.UnitAbbreviation, MU.SubUnitAbbreviation, ' +
...
//                       ' SSSSA.Size IN (SELECT Size FROM SizeRangeSizes WHERE Scale = P.SizeScale AND Range = P.SizeRange) AND ' +
                 ...
                 'GROUP BY PWK.Seq, PWK.Knife, PM.TableLength, PM.BAQuadraticA, PM.BAQuadraticB, PM.BAQuadraticC, PWK.Frequency, M.Type, M.Code, M.LinearAllowance, ' +
                          'SqFtPerPiece, M.SkinSize, M.Length, M.Width, M.CutType, M.Trimmed, M.SheetCutFromRoll, M.AreaCoeff, M.QualCoeff, MU.ToFeet, ' +
...
//                        ' / 12000.0), 0) AND ' +
      ...
      //                  'AND SSSCA.Size IN (SELECT Size FROM SizeRangeSizes WHERE Scale = P.SizeScale AND Range = P.SizeRange) AND ' +
                 ...
                 'GROUP BY PWK.Seq, PWK.Knife, PM.TableLength, PM.BAQuadraticA, PM.BAQuadraticB, PM.BAQuadraticC, PWK.Frequency, M.Type, M.Code, M.LinearAllowance, SqFtPerPiece, M.SkinSize, ' +
                          'M.Length, M.Width, M.CutType, M.Trimmed, M.SheetCutFromRoll, M.AreaCoeff, M.QualCoeff, MU.ToFeet, MU.SubUnitsPerUnit, MU.UnitAbbreviation, MU.SubUnitAbbreviation, KS.Type, KS.CutGap, ' +
...
var
  ...
  SqFtToUnits: real;
  RecCountModeTemp: TFDRecordCountMode;
...
begin
  ...
  SQLString := 'DECLARE Divisor Double, Multiplier Double; ' +
               ...
               '  M.AreaCoeff, M.QualCoeff, M.Trimmed, M.SheetCutFromRoll, MU.ToFeet, MU.SubUnitsPerUnit, MU.UnitAbbreviation, ' +
               ...
               '  MWidth = MatTab.Width; ' +
               '  MAdjLength = ROUND(IIF(MatTab."Type" = ''R'', ' + FloatToStrSQL(ROLLLENGTH_FT) + ' , (MatTab.Length * Multiplier)) / Divisor, 0); ' +
...
{               '     (SELECT Size ' +
  ...
  if (qBAForParts.RecordCount = qBAForParts.FetchOptions.RowsetSize) then
  ...
  //CJY: qBA.FetchOptions.RecordCountMode set to cmVisible (if required on the fly)
  //CJY: Fetching all records to correct qBA.RecordCount as uncertain of query input
  RecCountModeTemp := qBAForParts.FetchOptions.RecordCountMode;
  if not (qBAForParts.FetchOptions.RecordCountMode = cmVisible) then
  begin
    qBAForParts.FetchOptions.RecordCountMode := cmVisible;
    qBAForParts.Refresh;
  end;
  ...
  if not (qBAForParts.FetchOptions.RecordCountMode = RecCountModeTemp) then
  begin
    qBAForParts.FetchOptions.RecordCountMode := RecCountModeTemp;
    qBAForParts.Refresh;
  end;
...
begin
  ...
  try
    ...
    //CJY: qBA.FetchOptions.RecordCountMode set to cmVisible (if required on the fly)
    //CJY: Fetching all records to correct qBA.RecordCount as uncertain of query input
    RecCountModeTemp := qBA.FetchOptions.RecordCountMode;
    if not (qBA.FetchOptions.RecordCountMode = cmVisible) then
    begin
      qBA.FetchOptions.RecordCountMode := cmVisible;
      qBA.Refresh;
    end;
    if (qBA.RecordCount = qBA.FetchOptions.RowsetSize) then
      qBA.FetchAll;
    ...
    begin
      ...
      begin
        qKnifeSetExists.ParamByName('PartCode').AsString := PartCode;
        qKnifeSetExists.ParamByName('WidthNo').AsInteger := WidthNo;
...
//CJY I had removed this line to bypass an error. Not sure if it was a bug or for developing!!!
...
//          LinearMultiplier := qBA.FieldByName('SubUnitsPerUnit').Value / qBA.FieldByName('MatWidth').Value
  ...
  //Have to do this first and separately so the sizes of the following arrays can be set correctly.
    ...
    if not (qBA.FetchOptions.RecordCountMode = RecCountModeTemp) then
    begin
      qBA.FetchOptions.RecordCountMode := RecCountModeTemp;
      qBA.Refresh;
      if (qBA.RecordCount = qBA.FetchOptions.RowsetSize) then
        qBA.FetchAll;
    end;
