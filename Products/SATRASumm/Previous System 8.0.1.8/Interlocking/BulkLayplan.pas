...
type
  TfmBulkLayplan = class(TForm)
    ...
    procedure qLayplansAfterScroll(DataSet: TDataSet);
  private
...
begin
  ...
  begin
    ...
    //      gbProgress.Caption := 'Progress (' + inttostr(i) + ' of ' + inttostr(qLayplans.recordcount) + ' done)';
      ...
      if AbortBulkLayplan then
        MessageDlgPos('Bulk Layplanning Aborted', mtInformation, [mbOk], 0, ((Application.MainForm.Width div 3) + Application.MainForm.Left), ((Application.MainForm.Height div 3) + Application.MainForm.Top))
      else
        MessageDlgPos('Bulk Layplanning Complete', mtInformation, [mbOk], 0, ((Application.MainForm.Width div 3) + Application.MainForm.Left), ((Application.MainForm.Height div 3) + Application.MainForm.Top));
    end;
  end
  else
...
begin
  if BulkLayplanning or LoadingLayplans or (Pos('Loading...', Caption) > 0) then
  begin
  ...
  //caFree does not nil the object which is then
  //tested against on Close All of main form
  fmBulkLayplan := nil;
  end;
end;
...
procedure TfmBulkLayplan.qLayplansAfterScroll(DataSet: TDataSet);
begin
...
begin
  ...
  with qLayplans do
  begin
    if DetailsReady and (RecNo > 0) then
    begin
      qLayplansStatus.value := Details[RecNo - 1];
    end;
  end;
...
begin
  ...
  qLayplansPrepare.ParamByname('AllSizes').AsInteger := rgSizes.ItemIndex;
  qLayplansPrepare.ParamByname('PatternRes').AsInteger := varPatternRes;
  qLayplansPrepare.ParamByname('RollLengthFt').AsFloat := varRollLength_ft;
  ...
  if RightsProblem then
    MessageDlgPos('Rights are not granted for User - ' + SystemUserName, mtInformation, [mbOk], 0, ((Application.MainForm.Width div 3) + Application.MainForm.Left), ((Application.MainForm.Height div 3) + Application.MainForm.Top))
  //CJY qLayplans.FetchOptions.RecordCountMode set to cmTotal
  else if qLayplans.recordCount = 0 then
    MessageDlgPos('There are No Layplans required.', mtInformation, [mbOk], 0, ((Application.MainForm.Width div 3) + Application.MainForm.Left), ((Application.MainForm.Height div 3) + Application.MainForm.Top));
end;
