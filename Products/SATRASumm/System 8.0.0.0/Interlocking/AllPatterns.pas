...

{
  ...
  s := 'SELECT K.Code, K.SizeScale, K.MeasuredSize, K.Angle, KS.Description, KS.CutGap as KnifeCutGap, (' +
            'SELECT TOP 1 CONCAT(CAST(L.MaterialEdge AS SQL_VARCHAR), ''mm'') ' +
              'FROM LayplanSets L ' +
              'WHERE (L.KnifeCode = K.Code) AND (L.KnifeSizeScale = K.SizeScale) AND (L.KnifeSize = K.MeasuredSize) AND (L.MaterialLength = ' + FloatToStr(MatLength) + ') AND (L.MaterialWidth = ' + FloatToStr(MatWidth) +') AND (L.MaterialCutGap = ' + IntToStr(MatCutGap) + ') AND (L.MaterialCodeRestrictive ' + MatRes + ')) as MaterialEdge, (' +
            'SELECT IIF(COUNT(L.MaterialEdge) > 0, TRUE, FALSE) ' +
              'FROM LayplanSets L ' +
              'WHERE (L.KnifeCode = K.Code) AND (L.KnifeSizeScale = K.SizeScale) AND (L.KnifeSize = K.MeasuredSize) AND (L.MaterialLength = ' + FloatToStr(MatLength) + ') AND (L.MaterialWidth = ' + FloatToStr(MatWidth) +') AND (L.MaterialCutGap = ' + IntToStr(MatCutGap) + ') AND (L.MaterialCodeRestrictive ' + MatRes + ')) as Done ' +
       'FROM Knives K, KnifeSets KS ' +
       'WHERE (ManualEntry = False) AND ';
