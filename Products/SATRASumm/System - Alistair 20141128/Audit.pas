...
begin
  ...
  begin
    if messageDlg('Clear audit history?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
    begin
...
var
  OutFileC, OutFileD, OutFileP, OutFileU: TextFile;
...
begin
  ...
  begin
    ...
    try
      AssignFile(OutFileC, PathName + '\Cleared.asc');
      AssignFile(OutFileD, PathName + '\Deleted.asc');
      AssignFile(OutFileP, PathName + '\Printed.asc');
      AssignFile(OutFileU, PathName + '\Updated.asc');
      Rewrite(OutFileC);
      Rewrite(OutFileD);
      Rewrite(OutFileP);
      Rewrite(OutFileU);
      ...
      begin
        ...
        begin
          //Cleared
          if OldAudit then
            writeln(OutFileC, FullTicketNumber, ' ', FormatDateTime('dd/mm/yy', qAudit2.FieldByName('TransactionDate').value), ' ', Printed, ' ', Amended)
          else
            writeln(OutFileC, FullTicketNumber, ' ', FormatDateTime('dd/mm/yy', qAudit2.FieldByName('TransactionDate').value), ' ', Printed, ' ', Amended, ' ', TagNo, ' ', CustNo);
        end
        else if qAudit2.FieldByName('Type').value = 'D' then
        begin
          //Deleted
          if OldAudit then
            writeln(OutFileD, FullTicketNumber, ' ', FormatDateTime('dd/mm/yy', qAudit2.FieldByName('TransactionDate').value), ' ', Printed)
          else
            writeln(OutFileD, FullTicketNumber, ' ', FormatDateTime('dd/mm/yy', qAudit2.FieldByName('TransactionDate').value), ' ', Printed, ' ', TagNo, ' ', CustNo);
        end
        else if qAudit2.FieldByName('Type').value = 'P' then
        begin
          //Printed
          if OldAudit then
            writeln(OutFileP, FullTicketNumber, ' ', FormatDateTime('dd/mm/yy', qAudit2.FieldByName('TransactionDate').value), ' ', Material, ' ', Allowance, ' ', Construction, ' ', Part, ' ', TotalPairs)
          else
            writeln(OutFileP, FullTicketNumber, ' ', FormatDateTime('dd/mm/yy', qAudit2.FieldByName('TransactionDate').value), ' ', Material, ' ', Allowance, ' ', Construction, ' ', Part, ' ', TotalPairs, ' ', TagNo, ' ', CustNo);
        end
        else if qAudit2.FieldByName('Type').value = 'U' then
        begin
          //Updated
          if OldAudit then
            writeln(OutFileU, FullTicketNumber, ' ', FormatDateTime('dd/mm/yy', qAudit2.FieldByName('TransactionDate').value), ' ', Material, ' ', Allowance, ' ', Construction, ' ', Part, ' ', TotalPairs, ' ', Usage, ' ', Amended)
          else
            writeln(OutFileU, FullTicketNumber, ' ', FormatDateTime('dd/mm/yy', qAudit2.FieldByName('TransactionDate').value), ' ', Material, ' ', Allowance, ' ', Construction, ' ', Part, ' ', TotalPairs, ' ', Usage, ' ', Amended, ' ', TagNo, ' ', CustNo);
        end;
      ...
      CloseFile(OutFileP);
      CloseFile(OutFileU);
      messageDlg('Output files saved in directory ' + #13 +
                  PathName + '\', mtInformation, [mbOK], 0);

      Saved := True;
    except
      messagedlg('Error saving output files', mtinformation, [mbOk], 0);
    end;
    screen.cursor := crDefault;
  end
  else
    messageDlg('Parameters | Audit Path directory does not exist', mtInformation, [mbOK], 0);
