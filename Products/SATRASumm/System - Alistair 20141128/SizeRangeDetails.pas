...
begin
  ...
  begin
    if messagedlg('Save Changes to Size Range ' + SizeRangeCode + '?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
      btnSave.click
    else
      btnCancel.click;
  end;
...
begin
  try
    ...
    begin
      messagedlg('Size Range ' + SizeRangeCode + ' does not exist', mtInformation, [mbOk], 0);
      close;
    end;
  except
    Close;
...
begin
  ...
  if LockSuccess then
  begin
    CanDelete := (messagedlg('Delete Size Range?', mtConfirmation, [mbYes, mbNo], 0) = mrYes);
    if CanDelete then
    begin
      try
...
begin
  if not ExistingToFront('Where Used for Size Range', SizeRangeCode) then
  begin
    Screen.cursor := crHourGlass;
    Failed := False;
    try
...
begin
  ...
  tblSizeRanges.refresh;
  if tblSizeRanges.recordcount = 0 then
  begin
    messagedlg('Size Range ' + SizeRangeCode + ' has been deleted', mtInformation, [mbOk], 0);
    close;
  end
  else
  begin
    ...
    if ((Sender as TSpeedButton) = btnPrintPreview) then
      frSizeRanges.ShowPreparedReport
    else
      frSizeRanges.Print;
...
begin
  ...
  begin
    ...
    begin
      ...
      if InUse then
        messagedlg('Size Range ' + SizeRangeCode + ' used on a Size Relationship' + #13 +
                   'Can only change Description and Sample/Costed Sizes', mtInformation, [mbOk], 0);
    end;
...
begin
  ...
  begin
    ...
    begin
      messagedlg('Sample/Costed Sizes not in Size Range', mtInformation, [mbOk], 0);
      abort;
    end;
  end;
