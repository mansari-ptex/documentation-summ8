...
begin
  ...
  begin
    if messagedlg('Save Changes to Construction ' + ConstructionCode + '?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
      btnSave.click
    else
      btnCancel.click;
  end;
...
begin
  try
    ...
    begin
      messagedlg('Construction ' + ConstructionCode + ' does not exist', mtInformation, [mbOk], 0);
      close;
    end;
  except
    Close;
...
begin
  ...
  //If sizeRange is blank, Part must not exist...
    ...
    begin
      messagedlg('Part ' + tblConPartsPart.value + ' does not exist', mtInformation, [mbOk], 0);
      abort;
    end
    else if (tblConPartsPartSizeRange.value <> tblConstructionsSizeRange.value) then
    begin
      messagedlg('Cannot add a part with a different Size Range', mtInformation, [mbOk], 0);
      abort;
    end;
    ...
    begin
      messagedlg('Cannot add a part with a different Sample/Costed Size', mtInformation, [mbOk], 0);
      abort;
    end;
    if (tblConPartsPartMadeInPairs.value <> tblConstructionsMadeInPairs.value) then
    begin
      messagedlg('Cannot add a part with a different setting for Made in Pairs', mtInformation, [mbOk], 0);
      abort;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    if LockSuccess then
    begin
      CanDelete := (messagedlg('Delete Construction?', mtConfirmation, [mbYes, mbNo], 0) = mrYes);
      if CanDelete then
      begin
        try
...
begin
  ...
  begin
    if tblConParts.recordcount = 0 then
      MessageDlg('No Parts on Construction', mtInformation, [mbOK], 0)
    else
    begin
      Screen.cursor := crHourGlass;
      Failed := false;
...
begin
  ...
  if tblConstructions.recordcount = 0 then
  begin
    messagedlg('Construction ' + ConstructionCode + ' has been deleted', mtInformation, [mbOk], 0);
    close;
  end
  else
  begin
    ...
    if ((Sender as TSpeedButton) = btnPrintPreview) then
      frConstructions.ShowPreparedReport
    else
