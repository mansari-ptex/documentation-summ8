...
begin
  ...
  begin
    ...
    if WeekNo > 53 then
      messagedlg('Week Number is too big', mtInformation, [mbOK], 0)
    else if WeekNo < 1 then
      messagedlg('Week Number is too small', mtInformation, [mbOK], 0)
    else
    begin
      ...
      qNewTicketSequence.SQL.Text := 'INSERT INTO TicketSequences (WeekNo, SequenceNo, Style, Construction, Picture) ' +
                                     'SELECT ' + sWeekNo + ', ' + sSequenceNo + ', ''' + QS(Style) +
                                     ''', CurrentCon, Picture FROM Styles WHERE Style = ''' + QS(Style) + ''' ;' + #13 +
      ...
      begin
        messagedlg(StyleIncomplete + 'Ticket could not be created.', mtInformation, [mbOk], 0);
        if StyleFocus then
        begin
          eStyle.SetFocus;
          eStyle.SelectAll;
