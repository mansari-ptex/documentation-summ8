...
uses
  ...
  FireDAC.Stan.Def, FireDAC.Stan.Pool, FireDAC.Phys, DBGrids, frxReportPlus;
...
begin
  if Editing then
  begin
    if messagedlg('Save Changes to Cutter Names?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
      btnSave.click
    else
      btnCancel.click;
  end;
...
begin
  ...
  if ((Sender as TSpeedButton) = btnPrintPreview) then
    frCutterNames.ShowPreparedReport
  else
    frCutterNames.Print;
