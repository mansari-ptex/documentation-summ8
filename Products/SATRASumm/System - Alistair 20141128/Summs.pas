...
uses
  Windows, SysUtils, Classes, Controls, Forms, Dialogs, Menus, DB, StdCtrls,
  ...
  FireDAC.Phys.ADS, FireDAC.Comp.UI, frxReportPlus;
type
...
uses
  Messages, Analyse, AllParts, AllConstructions, AllKnifeSets, Audit, OpenKnife,
...
begin
  ...
  sbMain.Panels[4].text := 'Client : ' + Identifier;
end;
...
begin
  ...
  sbMain.Panels[3].text := 'User : ' + SystemUserName;
...
begin
  if (fmBulkLayplan <> nil) and (fmBulkLayplan.BulkLayplanning or fmBulkLayplan.LoadingLayplans) then
    messagedlg('Not available while Bulk Layplanning.', mtInformation, [mbOk], 0)
  else
  begin
    if MDIChildCount > 0 then
      repeat
        if MDIChildren[MDIChildCount - 1].Visible then
          MDIChildren[MDIChildCount - 1].Close;
        application.ProcessMessages;
      until MDIChildCount = 0;
  end;
end;
...
begin
  ...
  begin
    ...
    CloseMe := True;
    if messagedlg('Exit ' + SystemName + '?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
    begin
      setLength(Forms, 0);
...
begin
  ...
  sbMain.Panels[1].text := 'Server : ' + ConnectionSumms.Params.Values['Server'];
  sbMain.Panels[2].text := 'Connection : ' + ConnectionSumms.Params.Values['ServerTypes'];
end;
...
begin
  //Ensure Time visible
  sbMain.Panels[5].Text := DateTimeToStr(Now);
  ...
  lblTemp.caption := sbMain.Panels[5].text;
  sbMain.Panels[5].width := lblTemp.Width + 20;
  ...
  sbMain.Panels[4].width := sbMain.Panels[4].width + (RemainingWidth div 5);
...
begin
  ...
  begin
    NagAgain := False;
    messagedlg('Please insert ' + SystemName + ' dongle', mtInformation, [mbOk], 0);
    NagAgain := True;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      messagedlg('Close Pattern Assessment first', mtInformation, [mbOk], 0);
  end;
end;
...
begin
  ...
  begin
    if ExistingToFront('Synthetic Layplanning', '') then
      messagedlg('Synthetic Layplanning already in use.', mtInformation, [mbOk], 0)
    else
    begin
      Screen.cursor := crHourGlass;
      try
...
begin
  ...
  begin
    ...
    begin
      ...
      messagedlg('Close Synthetic Layplanning first', mtInformation, [mbOk], 0);
  end;
end;
