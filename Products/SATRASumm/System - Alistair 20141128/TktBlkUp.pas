...
uses
  ...
  FireDAC.Stan.Def, FireDAC.Stan.Pool, FireDAC.Phys, Vcl.DBGrids, TicketsAllDM;
...
begin
  ...
  begin
    DoBulkUpdate := False;
    messagedlg('No Tickets to Update', mtInformation, [mbOk], 0);
  end
  else
  begin
  //Note:  if statements are written with 2 ifs to prevent unnecessary accessing of tables.
    ...
    begin
      ...
      begin
        DoBulkUpdate := False;
        messagedlg('Material Price required', mtInformation, [mbOk], 0);
      end;
    ...
    begin
      ...
      begin
        DoBulkUpdate := False;
        messagedlg('Issued Allowance required', mtInformation, [mbOk], 0);
      end;
      tblTicketBulkUpdate.Filter := 'BulkUpdate = False';
    end;
    ...
    begin
      ...
      begin
        DoBulkUpdate := False;
        messagedlg('Sms required', mtInformation, [mbOk], 0);
      end;
      tblTicketBulkUpdate.Filter := 'BulkUpdate = False';
    end;
  end;
  ...
  begin
    ...
    if not SameMaterial then
      messagedlg('No common Material on selected Tickets', mtInformation, [mbOk], 0)
    else
    begin
      TotalAllowance := 0;
      TotalSms := 0;
