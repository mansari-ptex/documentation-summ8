...
begin
  ...
  //Filter Loops
    ...
    {$IFDEF DEBUGFULL}
      fmDebugger.DrawPointsB4Clockwise(OriginalPoints);
    {$ENDIF}
    MakeClockwise(OriginalPoints);
    {$IFDEF DEBUGFULL}
      fmDebugger.DrawPointsAfterClockwise(OriginalPoints);
    {$ENDIF}
  end;
...
//  {$ENDIF}
  ...
  {$IFDEF DEBUGFULL}
  fmDebugger.DrawExpansions(PatternPoints, ExpandedPoints);
  {$ENDIF}
  ...
  {$IFDEF DEBUGFULL}
  fmDebugger.DrawLoopsGone(ExpandedPoints);
  {$ENDIF}
...
begin
  ...
  {$IFDEF DEBUGFULL}
  fmDebugger.DrawEdges(Pattern, LeftsPattern, RightsPattern, No);
  {$ENDIF}
end;
...
begin
  ...
  {$IFDEF DEBUGFULL}
  fmDebugger.CheckForDuplicatePoints(Pattern, 'AddPointsEvery1 IN');
  {$ENDIF}
  ...
  //Check for Duplicate Points
  {$IFDEF DEBUGFULL}
  fmDebugger.CheckForDuplicatePoints(PatternOut, 'AddPointsEvery1 OUT');
  {$ENDIF}
...
begin
  ...
  begin
    ...
    ThisInterlock := OverlappingHullLeftAndRights(LeftsHull1, RightsHull1, LeftsHull2, RightsHull2, x, y, PatternMaxX2);
    fmDebugger.DrawOverlappingHullLeftAndRights(LeftsHull1, RightsHull1, LeftsHull2, RightsHull2, x, y, PatternMaxX2);
    {$ENDIF}
