...
uses
  ...
  FireDAC.Stan.Pool, FireDAC.Phys, Vcl.DBGrids, frxReportPlus;
...
begin
  ...
  begin
    if messagedlg('Save Changes to Material ' + MaterialCode + '?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
      btnSave.click
    else
      btnCancel.click;
  end;
...
begin
  try
    ...
    begin
      messagedlg('Material ' + MaterialCode + ' does not exist', mtInformation, [mbOk], 0);
      close;
    end
    else
    begin
...
begin
  ...
  if tblMaterials.recordcount = 0 then
  begin
    messagedlg('Material ' + MaterialCode + ' has been deleted', mtInformation, [mbOk], 0);
    close;
  end
  else
  begin
    ...
    if ((Sender as TSpeedButton) = btnPrintPreview) then
      frMaterialDetails.ShowPreparedReport
    else
