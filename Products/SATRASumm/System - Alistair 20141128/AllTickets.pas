...
begin
  ...
  begin
    ...
    begin
      btnGroup.Down := False;
      MessageDlg('No tickets to group', mtInformation, [mbOK], 0);
    end
    else
    begin
      //Attempt Lock
...
begin
  ...
  if ((Sender as TSpeedButton) = btnPrintPreview) then
    frAllTickets.ShowPreparedReport
  else
    frAllTickets.Print;
...
begin
  try
    ...
    if messageDlg('Delete selected Tickets?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
    begin
...
begin
  try
    ...
    if messageDlg('Clear selected Tickets?' + #13 + 'This may take a few minutes', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
    begin
      application.processmessages;
      screen.cursor := crHourGlass;
      qTicketSequences.DisableControls;
      ...
      begin
        ...
        try
          qGroupClear.ExecSQL;
          messageDlg('Clear completed', mtInformation, [mbOK], 0);
        except
          messagedlg('Clear failed', mtInformation, [mbOK], 0);
        end;
      end
      else
        messagedlg('No tickets selected', mtInformation, [mbOK], 0);
...
begin
  ...
  begin
    ...
    try
      ...
      begin
        ...
        if AllNotCreated then
          messagedlg('None of the selected tickets can be printed', mtWarning, [mbOk], 0)
        else
        begin
          ...
          if not(SelectedCount = qCountCreatedTicketsCreated.Value) then
            messagedlg('Not all of the selected tickets can be printed', mtWarning, [mbOk], 0);
        end;
    ...
    messagedlg('No tickets selected', mtWarning, [mbOk], 0);
end;
...
begin
  ...
  if FileSaved then
    messagedlg(FileName + ' file saved', mtInformation, [mbOK], 0);
end;
...
begin
  ...
  if SecondProcessInUse then
    messagedlg('Cannot close whilst creating tickets', mtInformation, [mbOk], 0);
end;
...
begin
  ...
  if ((Sender as TPBNumEdit).value < 1) or ((Sender as TPBNumEdit).value > 53) then
    (Sender as TPBNumEdit).Undo;
end;
...
begin
  ...
  if (ineEndSeq.value < 1) or (ineEndSeq.value > 32000) then
    ineEndSeq.Undo;
end;
...
begin
  ...
  if (ineStartSeq.value < 1) or (ineStartSeq.value > 32000) then
    ineStartSeq.Undo;
end;
