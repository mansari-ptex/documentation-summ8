...
uses
  ...
  FireDAC.Stan.Def, FireDAC.Stan.Pool, FireDAC.Phys, frxReportPlus;
...
begin
  if Editing then
  begin
    if messagedlg('Save Changes to Cutter Locations?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
      btnSave.click
    else
      btnCancel.click;
  end;
...
begin
  ...
  if ((Sender as TSpeedButton) = btnPrintPreview) then
    frCutterLocations.ShowPreparedReport
  else
