...
begin
  ...
  if ((Sender as TSpeedButton) = btnPrintPreview) then
    frAllKnifeSets.ShowPreparedReport
  else
...
begin
  ...
  if messageDlg('Delete selected KnifeSets?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
  begin
    ...
    messageDlg('Deletion complete.' + #13#13 + 'Note: Only Knife Sets that are not' + #13 + 'in use have been deleted.', mtInformation, [mbOK], 0);
  end;
