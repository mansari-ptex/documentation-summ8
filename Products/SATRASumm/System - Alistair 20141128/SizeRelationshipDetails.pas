...
begin
  ...
  begin
    if messagedlg('Save Changes to Size Relationship ' + SizeRelationshipCode + '?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
      btnSave.click
    else
      btnCancel.click;
  end;
...
begin
  try
    ...
    begin
      messagedlg('Size Relationship ' + SizeRelationshipCode + ' does not exist', mtInformation, [mbOk], 0);
      close;
    end;
  except
    Close;
...
begin
  ...
  if LockSuccess then
  begin
    CanDelete := (messagedlg('Delete Size Relationship?', mtConfirmation, [mbYes, mbNo], 0) = mrYes);
    if CanDelete then
    begin
      try
...
begin
  if not ExistingToFront('Where Used for Size Relationship', SizeRelationshipCode) then
  begin
    Screen.cursor := crHourGlass;
    Failed := False;
    try
...
begin
  ...
  begin
    messagedlg('Knife Size does not exist', mtInformation, [mbOk], 0);
    abort;
  end;
end;
...
begin
  ...
  tblSizeRelationships.refresh;
  if tblSizeRelationships.recordcount = 0 then
  begin
    messagedlg('Size Relationship ' + SizeRelationshipCode + ' has been deleted', mtInformation, [mbOk], 0);
    close;
  end
  else
  begin
    ...
    if ((Sender as TSpeedButton) = btnPrintPreview) then
      frSizeRelationship.ShowPreparedReport
    else
