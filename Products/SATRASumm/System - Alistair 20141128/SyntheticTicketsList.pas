...
uses
  ...
  XStringGridPlus, frxClass, frxDBSet, frxReportPlus;
...
begin
  ...
  if ((Sender as TSpeedButton) = btnPrintPreview) then
    frSyntheticsList.ShowPreparedReport
  else
...
begin
  ...
  begin
    ...
    fmSyntheticTicketsList.Width := 930;
  ...
  begin
    ...
    fmSyntheticTicketsList.Width := 576;
  end;
end;
...
begin
  ...
  s := 'SELECT P.WeekNo, P.SequenceNo, P.TicketNo, TS.MaterialCode, P.KnifeCode,' +
       '       TS.SizeScale, P.Size, ' +
       '       IIF(M.Type = ''R'', ' + FloatToStr(ROLLLENGTH_FT) + ' / (U.ToFeet / U.SubUnitsPerUnit), ' +
       '         M.Length) as Length, ' +
       '       M.Width as Width, ' +
       '       M.Cutgap, ' +
  ...
  begin
    ...
    begin
      ...
      try
        ...
        while (not qSyntheticTickets.eof) and fmCancelPrinting.visible do
        begin
          ...
          begin
            ...
            if LayplanLoaded then
              ...
              messagedlg('Layplan for Ticket ' + qSyntheticTicketsFullTicketNumber.value + ' is not available', mtInformation, [mbOk], 0);
          end;
