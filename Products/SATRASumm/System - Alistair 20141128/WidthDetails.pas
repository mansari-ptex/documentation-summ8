...
begin
  ...
  begin
    if messagedlg('Save Changes to Width Range ' + WidthRangeCode + '?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
      btnSave.click
    else
      btnCancel.click;
  end;
...
begin
  ...
  begin
    messagedlg('Width Range ' + WidthRangeCode + ' does not exist', mtInformation, [mbOk], 0);
    close;
  end;
end;
...
begin
  ...
  if LockSuccess then
  begin
    CanDelete := (messagedlg('Delete Width Range?', mtConfirmation, [mbYes, mbNo], 0) = mrYes);
    if CanDelete then
    begin
      try
...
begin
  tblWidthRanges.refresh;
  if tblWidthRanges.recordcount = 0 then
  begin
    messagedlg('Width Range ' + WidthRangeCode + ' has been deleted', mtInformation, [mbOk], 0);
    close;
  end
  else
  begin
