...
type
  TfmPartDetails = class(TForm)
    ...
    procedure dbeWidthRangeChange(Sender: TObject);
  private
...
begin
  ...
  begin
    if messagedlg('Save Changes to Part ' + PartCode + '?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
      btnSave.click
    else
      btnCancel.click;
  end;
...
begin
  try
    ...
    begin
      messagedlg('Part ' + PartCode + ' does not exist', mtInformation, [mbOk], 0);
      close;
    end
    else
    begin
      ...
      begin
        ...
        if DefaultSR = '' then
        begin
          messagedlg('Matching Size Relationship ' + tblPartsSizeRange.value + ' missing.' + #13 + 'This Size Relationship will be auto created for Costing system use.', mtInformation, [mbOk], 0);
...
procedure TfmPartDetails.dbeWidthRangeChange(Sender: TObject);
var
...
begin
  ...
  if tblParts.recordcount = 0 then
  begin
    messagedlg('Part ' + PartCode + ' has been deleted', mtInformation, [mbOk], 0);
    close;
  end
  else
  begin
    ...
    begin
      ...
      if ((Sender as TSpeedButton) = btnPrintPreview) or ((Sender as TSpeedButton) = btnPrintincAllowPreview) then
        frPartDetails.ShowPreparedReport
      else
        PrintingCancelled := not frPartDetails.Print;
    end
    else
      messagedlg('Cannot produce report.' + #13 + 'Allowance details unavailable for at least one width.' + #13 +
        'Open Parts/Part Allowance for further information', mtInformation, [mbOk], 0);
...
//      (tblPartWidthKnifeSizeRelationship.NewValue = tblPartWidthKnifeSizeRelationship.Value) then
    ...
    begin
      ...
      messagedlg('Please enter Knife and Size relationship before setting Master', mtInformation, [mbOK], 0);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      messagedlg('Unable to insert Sizes, Close Size Range and try again', mtInformation, [mbOk], 0);
      sysutils.abort;
    end;
  end;
...
begin
  ...
  begin
    ...
    if LockSuccess then
    begin
      CanDelete := (messagedlg('Delete Part?', mtConfirmation, [mbYes, mbNo], 0) = mrYes);
      if CanDelete then
      begin
        try
          tblParts.delete;
...
begin
  ...
  begin
    ...
    messagedlg('Cannot change Sample Size ' + #13 +
               'for a Part which is in use', mtInformation, [mbOk], 0);
  end;
end;
...
begin
  ...
  begin
    ...
    messagedlg('Cannot change Costed Size ' + #13 +
               'for a Part which is in use', mtInformation, [mbOk], 0);
  end;
end;
...
begin
  if PartInUse then
  begin
    messagedlg('Cannot change Made in Pairs ' + #13 +
               'for a Part which is in use', mtInformation, [mbOk], 0);
    dbcbMadeInPairs.OnClick := nil;
...
begin
  ...
  if tblKnifeSets.FindKey([Knife]) and tblKnifeSetsManualEntry.value then
    messagedlg('This a manual knife.', mtInformation, [mbOk], 0)
  else if Size <> '' then
  begin
    //Find Adjusted Size
    ...
    if (not AdjustedSizesValid) then
      messagedlg('No knives for Width or' + #13 + 'invalid adjusted size.', mtInformation, [mbOk], 0)
    else if not tblKnives.FindKey([Knife, Scale, AdjustedSize]) then
      messagedlg('Knife ' + Knife + ', Size ' + AdjustedSize + ' not found.', mtInformation, [mbOk], 0)
    else
    begin
      if (tblKnifeSetsCutGap.value > 0) and not(tblKnifeSetsCutGap.value = Cutgap) then
        messagedlg('The Cut gap on this Knife is ' + intToStr(tblKnifeSetsCutGap.Value) +
