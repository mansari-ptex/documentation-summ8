...
begin
  ...
  try
    ...
    begin
      messagedlg('No Knives selected', mtInformation, [mbOk],0);
      DoneMessage := False;
      Screen.cursor := crDefault;
      qListKnives.RecNo := 1; //CJY changed from qListKnives.First
      qListKnives.Prior; //CJY changed from qListKnives.First
    ...
    begin
      ...
      if NativeCode = 7057 then
        ...
        fmErrorHandler.DebugMessageDlg('Swap Error' + #13 + 'Nothing swapped', E.Message, qSwapKnives.Text);
      DoneMessage := False;
      Screen.cursor := crDefault;
  ...
  begin
    ...
    btnRefresh.Click;
    Screen.cursor := crDefault;
    messageDlg('Swap completed', mtInformation, [mbOK], 0);
    //CJY: qListKnives.FetchOptions.RecordCountMode set to cmTotal
    if qListKnives.RecordCount = 0 then
      close;
  end;
