...
uses
  ...
  FireDAC.Stan.Pool, FireDAC.Phys, TotalReport, frxReportPlus;
...
begin
  ...
  begin
    if messagedlg('Save Changes to Style ' + StyleCode + '?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
      btnSave.click
    else
      btnCancel.click;
  end;
...
begin
  try
    ...
    begin
      messagedlg('Style ' + StyleCode + ' does not exist', mtInformation, [mbOk], 0);
      close;
    end;
  except
    Close;
...
begin
  ...
  begin
    ...
    if LockSuccess then
    begin
      CanDelete := (messagedlg('Delete Style?', mtConfirmation, [mbYes, mbNo], 0) = mrYes);
      if CanDelete then
      begin
        tblStyles.Edit;
...
begin
  ...
  if tblStyles.recordcount = 0 then
  begin
    messagedlg('Style ' + StyleCode + ' has been deleted', mtInformation, [mbOk], 0);
    close;
  end
  else
  begin
...
begin
  ...
  begin
    if tblStyConstructions.RecordCount = 0 then
      messageDlg('No Constructions on Style', mtInformation, [mbOK], 0)
    else
    begin
      ...
      begin
        ...
        if NoParts = 0 then
          MessageDlg('No Parts on Construction ' + Code, mtInformation, [mbOK], 0)
        else
        begin
          Screen.cursor := crHourGlass;
          Failed := false;
...
begin
  ...
  begin
    ...
    begin
      messagedlg('Parameters | Style Pictures directory does not exist', mtInformation, [mbOK], 0);
...
begin
  ...
  begin
    if tblStylesCurrentCon.value = '' then
       messagedlg('No Current construction selected', mtWarning, [mbOk], 0);
...
begin
  ...
  //Create Whole Style Report for use with this Style
  try
