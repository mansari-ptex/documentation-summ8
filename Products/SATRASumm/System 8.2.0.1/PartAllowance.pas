...
uses
  ...
  ComCtrls, CmnTypes, Times_Summs7, Times_Summs8, Buttons, XStringGrid, XStringGridPlus,
  ToolWin, PartDetails, ActnList, frxClass, frxDBSet, Vcl.DBCtrls, Clipbrd,
  Vcl.DBGrids, frxReportPlus, CutUtils_Summs7, CutUtils_Summs8;
...
type
  TfmPartAllowance = class(TForm)
    ...
    dbgCuttingTimes_Summs8: TDBGridPlus;
    dsCuttingTimes_Summs8: TDataSource;
    ...
    qCuttingTimesSample_Summs8: TFDQueryPlus;
    qCuttingTimesCosted_Summs8: TFDQueryPlus;
    ...
    frTimesBreakdown_Summs8: TfrxReportPlus;
    frudsPartAllowance: TfrxUserDataSet;
    frdbTimes_Summs8: TfrxDBDataset;
    qCuttingTimesCosted_Summs8Minutes: TFloatField;
    qCuttingTimesCosted_Summs8Freq: TFMTBCDField;
    qCuttingTimesCosted_Summs8Per: TFMTBCDField;
    qCuttingTimesCosted_Summs8BatchFreq: TFMTBCDField;
    qCuttingTimesCosted_Summs8Time: TFloatField;
    qCuttingTimesCosted_Summs8PerWhat: TStringField;
    ...
    qGetDataSheetCutFromRoll: TBooleanField;
    qCuttingTimesSample_Summs7: TFDQueryPlus;
    qCuttingTimesSample_Summs7Seq: TIntegerField;
    qCuttingTimesSample_Summs7Category: TStringField;
    qCuttingTimesSample_Summs7Element: TStringField;
    qCuttingTimesSample_Summs7Description: TStringField;
    qCuttingTimesSample_Summs7BatchFreq: TFMTBCDField;
    qCuttingTimesSample_Summs7Time: TFloatField;
    qCuttingTimesSample_Summs8Seq: TIntegerField;
    qCuttingTimesSample_Summs8Category: TStringField;
    qCuttingTimesSample_Summs8Element: TStringField;
    qCuttingTimesSample_Summs8Description: TStringField;
    qCuttingTimesSample_Summs8Minutes: TFloatField;
    qCuttingTimesSample_Summs8Freq: TFMTBCDField;
    qCuttingTimesSample_Summs8Per: TFMTBCDField;
    qCuttingTimesSample_Summs8PerWhat: TStringField;
    qCuttingTimesSample_Summs8BatchFreq: TFMTBCDField;
    qCuttingTimesSample_Summs8Time: TFloatField;
    dbgCuttingTimes_Summs7: TDBGridPlus;
    qCuttingTimesCosted_Summs7: TFDQueryPlus;
    ...
    dsCuttingTimes_Summs7: TDataSource;
    qCuttingTimesSample_Summs7Bms: TFloatField;
    qCuttingTimesCosted_Summs7Bms: TFloatField;
    qCuttingTimesSample_Summs7AdjBms: TFloatField;
    qCuttingTimesCosted_Summs7AdjBms: TFloatField;
    qCuttingTimesSample_Summs7Adj: TFMTBCDField;
    qCuttingTimesCosted_Summs7Adj: TFMTBCDField;
    qCuttingTimesSample_Summs7PerBatchYesNo: TStringField;
    qCuttingTimesCosted_Summs7PerBatchYesNo: TStringField;
    qCuttingTimesSample_Summs7PerBatch: TIntegerField;
    qCuttingTimesCosted_Summs7PerBatch: TIntegerField;
    frdbTimes_Summs7: TfrxDBDataset;
    frTimesBreakdown_Summs7: TfrxReportPlus;
    ...
    procedure frPartAllowanceGetValue(const VarName: string;
      var Value: Variant);
    procedure frTimesBreakdown_Summs8BeforePrint(Sender: TfrxReportComponent);
    procedure frTimesBreakdown_SummsGetValue(const VarName: string;
      var Value: Variant);
    procedure qCuttingTimesSample_Summs8CalcFields(DataSet: TDataSet);
    procedure qCuttingTimesCosted_Summs8CalcFields(DataSet: TDataSet);
    procedure btnSizeClick(Sender: TObject);
    procedure qCuttingTimesSample_Summs7CalcFields(DataSet: TDataSet);
    procedure qCuttingTimesCosted_Summs7CalcFields(DataSet: TDataSet);
    procedure frTimesBreakdown_Summs7BeforePrint(Sender: TfrxReportComponent);
  private
    ...
    LegacyCuttingTimesUsed: Boolean;
  public
    { Public declarations }
...
begin
  if BusyPrinting or LoadingAllowance or (Pos('Loading...', Caption) > 0) then
  begin
...
//Attempted capture Block
try
  Success := Allowances(SuppressMessage);
except
  on e: Exception do
  begin
    raise Exception.Create(e.Message + ' PPNW');
  end;
end;
...
begin
  ...
  begin
    if LegacyCuttingTimesUsed then
      frTimesBreakdown_Summs7.ReportOptions.Name := 'Preview ' + Caption
    else
      frTimesBreakdown_Summs8.ReportOptions.Name := 'Preview ' + Caption;
    ClosePreviewForm(Caption);

    if rgSampleCosted.ItemIndex = 0 then
    begin
      if LegacyCuttingTimesUsed then
        frdbTimes_Summs7.DataSet := qCuttingTimesSample_Summs7
      else
        frdbTimes_Summs8.DataSet := qCuttingTimesSample_Summs8;
    end
    else
    begin
      if LegacyCuttingTimesUsed then
        frdbTimes_Summs7.DataSet := qCuttingTimesCosted_Summs7
      else
        frdbTimes_Summs8.DataSet := qCuttingTimesCosted_Summs8;
    end;

    if LegacyCuttingTimesUsed then
    begin
      MyBookmark := frdbTimes_Summs7.DataSet.GetBookmark;
      frdbTimes_Summs7.DataSet.DisableControls;

      frTimesBreakdown_Summs7.PrintOptions.PrintMode := pmScale;
      frTimesBreakdown_Summs7.PrintOptions.PrintOnSheet := GetPaperSize;
      frTimesBreakdown_Summs7.PrepareReport;

      try
        frdbTimes_Summs7.DataSet.GotoBookmark(MyBookmark);
      except
      end;
      frdbTimes_Summs7.DataSet.EnableControls;
      frdbTimes_Summs7.DataSet.FreeBookmark(MyBookmark);
    end
    else
    begin
      MyBookmark := frdbTimes_Summs8.DataSet.GetBookmark;
      frdbTimes_Summs8.DataSet.DisableControls;

      frTimesBreakdown_Summs8.PrintOptions.PrintMode := pmScale;
      frTimesBreakdown_Summs8.PrintOptions.PrintOnSheet := GetPaperSize;
      frTimesBreakdown_Summs8.PrepareReport;

      try
        frdbTimes_Summs8.DataSet.GotoBookmark(MyBookmark);
      except
      end;
      frdbTimes_Summs8.DataSet.EnableControls;
      frdbTimes_Summs8.DataSet.FreeBookmark(MyBookmark);
    end;
    ...
    begin
      if LegacyCuttingTimesUsed then
      begin
        with frTimesBreakdown_Summs7 do
        begin
          ShowPreparedReport;
          if (((PreviewForm.Left + (PreviewForm.Width div 2)) > Application.MainForm.Width) or
              (((PreviewForm.Left + (PreviewForm.Width div 2)) < 0))) then
      ...
      begin
        with frTimesBreakdown_Summs8 do
        begin
          ShowPreparedReport;
          if (((PreviewForm.Left + (PreviewForm.Width div 2)) > Application.MainForm.Width) or
              (((PreviewForm.Left + (PreviewForm.Width div 2)) < 0))) then
    ...
    begin
      if LegacyCuttingTimesUsed then
        frTimesBreakdown_Summs7.Print
      else
        frTimesBreakdown_Summs8.Print;
    end;
  end;
...
begin
  ...
  LegacyCuttingTimesUsed := LegacyCuttingTimes;
...
begin
  ...
  begin
    dbgCuttingTimes_Summs7.Visible := LegacyCuttingTimesUsed;
    dbgCuttingTimes_Summs8.Visible := not LegacyCuttingTimesUsed;

    if LegacyCuttingTimesUsed then
    begin
      dbgCuttingTimes_Summs7.align := alClient;
      dbgCuttingTimes_Summs7.Columns[7].Width := 65; //Without this line, whatever this is set to in environment it returns to 190 - why?!
    end
    else
    begin
      dbgCuttingTimes_Summs8.align := alClient;
      dbgCuttingTimes_Summs8.Columns[7].Width := 65; //Without this line, whatever this is set to in environment it returns to 190 - why?!
    end;
...
begin
  ...
  begin
    if rgSampleCosted.ItemIndex = 0 then
      dsCuttingTimes_Summs7.dataSet := qCuttingTimesSample_Summs7
    else
      dsCuttingTimes_Summs7.dataSet := qCuttingTimesCosted_Summs7;
    TotalCuttingTimes(dsCuttingTimes_Summs7.dataSet as TFDQueryPlus);
  end
  else
  begin
    if rgSampleCosted.ItemIndex = 0 then
      dsCuttingTimes_Summs8.dataSet := qCuttingTimesSample_Summs8
    else
      dsCuttingTimes_Summs8.dataSet := qCuttingTimesCosted_Summs8;
    TotalCuttingTimes(dsCuttingTimes_Summs8.dataSet as TFDQueryPlus);
  end;
end;
...
begin
  ...
  begin
    //CJY: If CalculateTime finds a min/max error set time to 'Unavailable' and hide times
    if LegacyCuttingTimes then
    begin
      dmTimes_Summs7.CalculateTime(TRUE, qGetData, qCuttingTimesSample_Summs7, SampleAdjInterlocks);
      CuttingTimesAvailable := TRUE;
    end
    else
    begin
      dmTimes_Summs8.CalculateTime(TRUE, qGetData, qCuttingTimesSample_Summs8, SampleAdjInterlocks);
    ...
    begin
      if LegacyCuttingTimes then
        sgAllowances.Cells[1, sgAllowances.RowCount - 1] := FourDps(TotalCuttingTimes(qCuttingTimesSample_Summs7))
      else
        sgAllowances.Cells[1, sgAllowances.RowCount - 1] := FourDps(TotalCuttingTimes(qCuttingTimesSample_Summs8))
    end
    else
      sgAllowances.Cells[1, sgAllowances.RowCount - 1] := 'Unavailable';
//
...
//    qCuttingTimes.Open;
//    sgAllowances.Cells[1, sgAllowances.RowCount - 1] := FourDps(qCuttingTimes.FieldByName('Time').value);
//    CuttingTimesAvailable := True;
  end;
  ...
  begin
    ...
    if LegacyCuttingTimes then
    begin
      dmTimes_Summs7.CalculateTime(FALSE, qGetData, qCuttingTimesCosted_Summs7, CostedAdjInterlocks);
      CuttingTimesAvailable := TRUE;
    end
    else
    begin
      dmTimes_Summs8.CalculateTime(FALSE, qGetData, qCuttingTimesCosted_Summs8, CostedAdjInterlocks);
      CuttingTimesAvailable := not dmCutUtils_Summs8.MessageShown;
    end;
    ...
    begin
      if LegacyCuttingTimes then
        sgAllowances.Cells[2, sgAllowances.RowCount - 1] := FourDps(TotalCuttingTimes(qCuttingTimesCosted_Summs7))
      else
        sgAllowances.Cells[2, sgAllowances.RowCount - 1] := FourDps(TotalCuttingTimes(qCuttingTimesCosted_Summs8))
    end
    else
      sgAllowances.Cells[2, sgAllowances.RowCount - 1] := 'Unavailable';
//
...
//    qCuttingTimes.Open;
//    sgAllowances.Cells[2, sgAllowances.RowCount - 1] := FourDps(qCuttingTimes.FieldByName('Time').value);
//    CuttingTimesAvailable := True;
  end;
...
var
  ...
  iQuietCounter: integer;
  sSuspectError: string;

begin
//Capture Attempt Block 1 Start
iQuietCounter := 0;
try
//Capture Attempt Block 1 End
...
iQuietCounter := 20;
  ...
  begin
    ...
    if not SuppressMessage then
      MessageDlgPos(Error, mtInformation, [mbOk], 0, ((Application.MainForm.Width div 3) + Application.MainForm.Left), ((Application.MainForm.Height div 3) + Application.MainForm.Top));
...
//Capture Attempt Block 2 Start
except
  on e: Exception do
  begin
    sSuspectError := '';

    if not Assigned(dmBasAll) then
      sSuspectError := sSuspectError + ' dmBasAll';

    if not Assigned(dmAdjFact) then
      sSuspectError := sSuspectError + ' dmAdjFact';

    raise Exception.Create(E.Message + ' ' + IntToStr(iQuietCounter) + sSuspectError + ' ' + Error);
  end;
end;
//Capture Attempt Block 2 End
...
begin
  if sgAllowances.Cells[2, 1] = 'Unavailable' then
    MessageDlgPos('Costed allowance is unavailable', mtInformation, [mbOk], 0, ((Application.MainForm.Width div 3) + Application.MainForm.Left), ((Application.MainForm.Height div 3) + Application.MainForm.Top))
  else
  begin
...
procedure TfmPartAllowance.qCuttingTimesCosted_Summs7CalcFields(DataSet: TDataSet);
begin
  if (DataSet as TFDQueryPlus).FieldByName('PerBatch').Value = 1 then
    (DataSet as TFDQueryPlus).FieldByName('PerBatchYesNo').value := 'Yes'
  else
    (DataSet as TFDQueryPlus).FieldByName('PerBatchYesNo').value := 'No';
end;

procedure TfmPartAllowance.qCuttingTimesCosted_Summs8CalcFields(DataSet: TDataSet);
begin
  (DataSet as TFDQueryPlus).FieldByName('PerWhat').Value := 'Shoe(s)';
end;

procedure TfmPartAllowance.qCuttingTimesSample_Summs7CalcFields(DataSet: TDataSet);
begin
  if (DataSet as TFDQueryPlus).FieldByName('PerBatch').Value = 1 then
    (DataSet as TFDQueryPlus).FieldByName('PerBatchYesNo').value := 'Yes'
  else
...
procedure TfmPartAllowance.qCuttingTimesSample_Summs8CalcFields(DataSet: TDataSet);
begin
  (DataSet as TFDQueryPlus).FieldByName('PerWhat').Value := 'Shoe(s)';
end;
...
procedure TfmPartAllowance.frTimesBreakdown_Summs7BeforePrint(Sender: TfrxReportComponent);
begin
  frTimesBreakdown_Summs7.PreviewOptions.AllowEdit := False;
  frTimesBreakdown_Summs7.PreviewOptions.Buttons := frTimesBreakdown_Summs7.PreviewOptions.Buttons - [pbEdit, pbOutline, pbFind];
  frTimesBreakdown_Summs7.PreviewOptions.Maximized := fmSumms.mmPrintPreviewMax.checked;
  if fmSumms.mmPrintPreview100.checked then
    frTimesBreakdown_Summs7.PreviewOptions.ZoomMode := zmDefault
  else
    frTimesBreakdown_Summs7.PreviewOptions.ZoomMode := zmWholePage;
end;

procedure TfmPartAllowance.frTimesBreakdown_Summs8BeforePrint(Sender: TfrxReportComponent);
begin
  frTimesBreakdown_Summs8.PreviewOptions.AllowEdit := False;
  frTimesBreakdown_Summs8.PreviewOptions.Buttons := frTimesBreakdown_Summs8.PreviewOptions.Buttons - [pbEdit, pbOutline, pbFind];
  frTimesBreakdown_Summs8.PreviewOptions.Maximized := fmSumms.mmPrintPreviewMax.checked;
  if fmSumms.mmPrintPreview100.checked then
    frTimesBreakdown_Summs8.PreviewOptions.ZoomMode := zmDefault
  else
    frTimesBreakdown_Summs8.PreviewOptions.ZoomMode := zmWholePage;
end;

procedure TfmPartAllowance.frTimesBreakdown_SummsGetValue(const VarName: string;
  var Value: Variant);
var
  EndTitle: string;
