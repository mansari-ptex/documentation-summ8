...
type
  TfmAllPatterns = class(TForm)
    ...
    lblSearch: TLabel;
    qKnivesMaterialEdge: TStringField;
...
begin
  if {$IFDEF SATRASUMM} Option_LegacySynthetics and {$ENDIF} (qKnivesKnifeCutGap.Value <> MatCutGap) then
    MessageDlgPos('The Cut gap on this Knife is ' + intToStr(qKnivesKnifeCutGap.Value) +
  ...
  if ((IntToStr(MatEdge) + 'mm') <> qKnivesMaterialEdge.value) and (qKnivesMaterialEdge.value <> '') then
  begin
    if MessageDlgPos('Selecting this lay plan will change the Material Edge' + #13 + 'from ' + IntToStr(MatEdge) + 'mm to ' + qKnivesMaterialEdge.value + #13 + #13 + 'Continue?',
                  mtConfirmation, [mbYes, mbNo], 0, ((Application.MainForm.Width div 3) + Application.MainForm.Left), ((Application.MainForm.Height div 3) + Application.MainForm.Top)) = mrYes then
    begin
      MatEdge := StrToIntDef(StringReplace(qKnivesMaterialEdge.value, 'm', '', [rfReplaceAll, rfIgnoreCase]), MatEdge);
      ModalResult := mrYes;
    end
    else
      ModalResult := mrOK;
...

{
  s := 'SELECT K.Code, K.SizeScale, K.MeasuredSize, K.Angle, KS.Description, KS.CutGap as KnifeCutGap, ' +
       ...
       'WHERE (L.KnifeCode = K.Code) AND (L.KnifeSizeScale = K.SizeScale) AND (L.KnifeSize = K.MeasuredSize) AND ' +
			       '(L.MaterialLength = ' + FloatToStrSQL(MatLength) + ') AND (L.MaterialWidth = ' + FloatToStrSQL(MatWidth) +
  ...
  s := 'SELECT K.Code, K.SizeScale, K.MeasuredSize, K.Angle, KS.Description, KS.CutGap as KnifeCutGap, (' +
            'SELECT TOP 1 CONCAT(CAST(L.MaterialEdge AS SQL_VARCHAR), ''mm'') ' +
              'FROM LayplanSets L ' +
              'WHERE (L.KnifeCode = K.Code) AND (L.KnifeSizeScale = K.SizeScale) AND (L.KnifeSize = K.MeasuredSize) AND (L.MaterialLength = ' + FloatToStrSQL(MatLength) + ') AND (L.MaterialWidth = ' + FloatToStrSQL(MatWidth) +') AND (L.MaterialCutGap = ' + IntToStr(MatCutGap) + ') AND (L.MaterialCodeRestrictive ' + MatRes + ')) as MaterialEdge, (' +
            'SELECT IIF(COUNT(L.MaterialEdge) > 0, TRUE, FALSE) ' +
              'FROM LayplanSets L ' +
              'WHERE (L.KnifeCode = K.Code) AND (L.KnifeSizeScale = K.SizeScale) AND (L.KnifeSize = K.MeasuredSize) AND (L.MaterialLength = ' + FloatToStrSQL(MatLength) + ') AND (L.MaterialWidth = ' + FloatToStrSQL(MatWidth) +') AND (L.MaterialCutGap = ' + IntToStr(MatCutGap) + ') AND (L.MaterialCodeRestrictive ' + MatRes + ')) as Done ' +
       'FROM Knives K, KnifeSets KS ' +
       'WHERE (ManualEntry = False) AND ';
...
begin
  ...
  begin
    ...
    begin
      SortOrder(dbgKnives, Column, SQLString);
