object fmBrowseWidthRanges: TfmBrowseWidthRanges
  Left = 431
  Top = 186
  BorderStyle = bsDialog
  Caption = 'Browse Width Ranges'
  ClientHeight = 366
  ClientWidth = 454
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = True
  Position = poScreenCenter
  OnCreate = FormCreate
  OnHide = FormHide
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object tbMain: TPanel
    Left = 0
    Top = 0
    Width = 454
    Height = 29
    Align = alTop
    Color = clAqua
    TabOrder = 0
    object btnRefresh: TSpeedButton
      Left = 3
      Top = 0
      Width = 23
      Height = 23
      Hint = 'Refresh'
      AllowAllUp = True
      Flat = True
      Glyph.Data = {
        
        3393333888333888338333333999993333333333388888333333}
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = True
      OnClick = btnRefreshClick
    end
  end
  object pnlSearch: TPanel
    Left = 0
    Top = 29
    Width = 454
    Height = 41
    Align = alTop
    BevelOuter = bvNone
    ParentColor = True
    TabOrder = 1
    object lblWidthRange: TLabel
      Left = 263
      Top = 14
      Width = 70
      Height = 13
      Caption = 'lblWidthRange'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clFuchsia
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      Visible = False
    end
    object lblSearch: TLabel
      Left = 3
      Top = 14
      Width = 34
      Height = 13
      Caption = 'Search'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clFuchsia
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object edSearch: TEdit
      Left = 50
      Top = 10
      Width = 180
      Height = 21
      CharCase = ecUpperCase
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      MaxLength = 20
      ParentFont = False
      TabOrder = 0
      OnChange = edSearchChange
      OnKeyPress = edSearchKeyPress
    end
  end
  object dbgWidthRanges: TDBGridPlus
    AlignWithMargins = True
    Left = 3
    Top = 73
    Width = 448
    Height = 290
    Align = alClient
    BiDiMode = bdLeftToRight
    BorderStyle = bsNone
    Color = clAqua
    DataSource = dsWidthRanges
    DrawingStyle = gdsClassic
    FixedColor = clLime
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick]
    ParentBiDiMode = False
    ParentFont = False
    ReadOnly = True
    TabOrder = 2
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clFuchsia
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
    OnCellClick = dbgWidthRangesCellClick
    OnDblClick = dbgWidthRangesDblClick
    OnKeyPress = dbgWidthRangesKeyPress
    OnTitleClick = dbgWidthRangesTitleClick
    Columns = <
      item
        Expanded = False
        FieldName = 'Code'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        Width = 180
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Description'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        Width = 250
        Visible = True
      end>
  end
  object dsWidthRanges: TDataSource
    DataSet = qWidthRanges
    Left = 114
    Top = 90
  end
  object qWidthRanges: TFDQueryPlus
    Connection = fmSumms.ConnectionSumms
    SQL.Strings = (
      
        'SELECT Code,Description FROM WRngs WHERE Code LIKE :Search ORDER' +
        ' BY Code, Code')
    Left = 193
    Top = 100
    ParamData = <
      item
        Name = 'Search'
        ParamType = ptInput
      end>
    object qWidthRangesCode: TStringField
      FieldName = 'Code'
    end
    object qWidthRangesDescription: TStringField
      FieldName = 'Description'
      Size = 30
    end
  end
end
