object fmAllPatterns: TfmAllPatterns
  ...
  object qKnives: TFDQueryPlus
    ...
    SQL.Strings = (
        ...
        '      (SELECT TOP 1 CONCAT(CAST(L.MaterialEdge AS SQL_VARCHAR), ' +
        #39'mm'#39') '
      ' '#9'              FROM Layplans L'
        ...
        #9#9#9#9'        (L.MaterialCodeRestrictive = :MatRestrictive)), TRUE' +
        ', FALSE)) as MaterialEdge'
      '      (SELECT IIF(COUNT(L.KnifeCode) > 0, TRUE, FALSE)'
      ' '#9'              FROM Layplans L'
    ...
    object qKnivesMaterialEdge: TStringField
      FieldName = 'MaterialEdge'
      Size = 15
    end
  end
