    ...
    begin
      ...
      begin
        ...
        if ((Distance(Pattern2D[j].x, Pattern2D[j].y, IntPoint.x, IntPoint.y) <= jDist) and   //point is between j's
          ...
          (FoundIntersection or (not((Testi2 = j) and jWithinDup)) and (not((Testj2 = i) and j2WithinDup)))) then
//This alteration will allow any secondary intersections to test for validity. This is to capture
//  rare cases where a false intersection has allowed a spike to be removed from the shape.
//          (not((Testi2 = j) and jWithinDup)) and (not((Testj2 = i) and j2WithinDup))) then
  ...
  begin
  {$IFDEF DEBUGFULL}
    if Assigned(fmDebugger) then fmDebugger.DrawIntLines(Pattern, ClosestPointToInt, round(PatternInters.Intersected.x), round(PatternInters.Intersected.y));
...
//    DUPLICATELIMIT and puts any SegmentMidPoint along the line inside the polygon.
      ...
      begin
        PatternSetB[k].x := Pattern[DuplicatePointIndex].x;
        PatternSetB[k].y := Pattern[DuplicatePointIndex].y;
      end;
...
begin
  ...
  {$IFDEF DEBUGFULL}
    if Assigned(fmDebugger) then fmDebugger.DrawPattern(Pattern);
  {$ENDIF}
  ...
  {$IFDEF DEBUGFULL}
    if Assigned(fmDebugger) then fmDebugger.DrawPatternSpikesGone(Pattern);
  {$ENDIF}
  ...
  {$IFDEF DEBUGFULL}
    if Assigned(fmDebugger) then fmDebugger.DrawLoopFreeFirstExp(Pattern, ExpandedPattern);
    if Assigned(fmDebugger) then fmDebugger.Show;
  {$ENDIF}
