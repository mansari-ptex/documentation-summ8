...
uses
  ...
  BrowseSizeScales, BrowseSizeRelationships, AdvErrorHandler, KnifeNettArea;
...
//CJY Manual without measuered size
  ...
  begin
    ...
    SizesArray[Length(SizesArray) - 1] := QS(cbMeasuredSize.Text);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        SetLength(SizesArray, Length(SizesArray) + 1);
        SizesArray[Length(SizesArray) - 1] := QS(cbMeasuredSize.Items[i]);
    ...
    begin
      ...
      MessageDlgPos('Not enough sizes in Relationship', mtInformation, [mbOK], 0, ((Application.MainForm.Width div 3) + Application.MainForm.Left), ((Application.MainForm.Height div 3) + Application.MainForm.Top));
      abort;
    end;
  end;
...
//        dmKnifeNettArea.NettArea(QS(Code), QS(KnifeSizeScale), SizesArray[i]);
        //Must NOT pass QS() here
        dmKnifeNettArea.NettArea(Code, KnifeSizeScale, SizesArray[i]);
...
procedure TfmNewKnifeSet.LocalConnectionSummsAfterConnect(Sender: TObject);
begin
...
begin
  if Auto then
  begin
    if fmBrowseSizeRelationships.ShowModal = mrOK then
      eSizeScale.text := fmBrowseSizeRelationships.lblRelationship.caption;
  end
  else
  begin
    if fmBrowseSizeScales.ShowModal = mrOK then
...
begin
  ...
  begin
    ...
    begin
      ...
      MeasuredSizeInScale := False;
      with cbMeasuredSize do
      begin
        for i := 0 to Items.Count - 1 do
        begin
          if Items[i] = KnifeMeasuredSize then
            MeasuredSizeInScale := True;
        end;

        if not MeasuredSizeInScale then
          Text := Items[0]
        else
          Text := KnifeMeasuredSize;
      end;
    end;
  end;
end;
