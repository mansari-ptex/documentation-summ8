...
uses
  ...
  ComCtrls, DB, FireDAC.Stan.Intf, FireDAC.Stan.Option, FireDAC.Stan.Param,FireDAC.Stan.Error, FireDAC.DatS, FireDAC.Phys.Intf, FireDAC.DApt.Intf,FireDAC.Stan.Async, FireDAC.DApt, FireDAC.Comp.DataSet, FireDAC.Comp.Client, FDQueryPlus, FDTablePlus,   Interlocking, Results,
...
function TfmCaller.OverlappingHulls: Boolean;
var
...
procedure TfmCaller.UpdateResults(Interlock: TInterlock; Knife2: TPattern);
var
...
procedure TfmCaller.DisplayResults(RemHeight, RemWidth: integer; LayPlan: Boolean);
var
  ClipRect: TRect;
  bmpResults: TBitmap;
...
procedure TfmCaller.btnAllKnivesClick(Sender: TObject);
var
  NumPoints: integer;
  Tolerance: TFloat;
...
//  else
//    MessageDlgPos('This is a manual knife', mtInformation, [mbOK], 0, ((Application.MainForm.Width div 3) + Application.MainForm.Left), ((Application.MainForm.Height div 3) + Application.MainForm.Top));
...
function TfmCaller.ReadPattern(Code, Size: string): TPointArray;
var
  Pattern: TPointArray;
  i: integer;
...
procedure TfmCaller.btnW1W2Click(Sender: TObject);
var
...
begin
  ...
  if (not BestInterlockW1.Found) and (not BestInterlockW2.Found) then
    MessageDlgPos('No Soln', mtInformation, [mbOk], 0, ((Application.MainForm.Width div 3) + Application.MainForm.Left), ((Application.MainForm.Height div 3) + Application.MainForm.Top))
  else
  begin
...
procedure TfmCaller.btnW1Click(Sender: TObject);
var
...
procedure TfmCaller.btnW2Click(Sender: TObject);
var
  BestInterlock: TInterlock;
  s: string;
...
procedure TfmCaller.btnGangClick(Sender: TObject);
var
...
procedure TfmCaller.cbOriginalHullTakeOutClick(Sender: TObject);
begin
  if cbOriginalHullTakeOut.Checked then
    cbSpeedRotate.Checked := False;
end;

procedure TfmCaller.cbSpeedRotateClick(Sender: TObject);
begin
  if cbSpeedRotate.Checked then
    cbOriginalHullTakeOut.Checked := False;
end;

procedure TfmCaller.cbTrueSizeClick(Sender: TObject);
begin
  fmDebugger.TrueSize := cbTrueSize.Checked;
end;

procedure TfmCaller.btnAllKnives2Click(Sender: TObject);
var
  OriginalPoints: TPointArray;
...
procedure TfmCaller.btnCancelClick(Sender: TObject);
begin
  LeatherCancel := True;
end;

procedure TfmCaller.btnCopyAngleClick(Sender: TObject);
begin
  sedtAngle2.value := sedtAngle.Value;
end;

procedure TfmCaller.FormCreate(Sender: TObject);
begin
...
procedure TfmCaller.btnSATRASummLeatherClick(Sender: TObject);
var
...
//  GrossArea := KnivesUsed[0, 0].GrossArea;
  ...
  if LeatherCancel then
    MessageDlgPos('Aborted', mtInformation, [mbOk], 0, ((Application.MainForm.Width div 3) + Application.MainForm.Left), ((Application.MainForm.Height div 3) + Application.MainForm.Top));
...
procedure TfmCaller.btnDrawClick(Sender: TObject);
begin
  DisplayResults(RemH, RemW, False);
end;

procedure TfmCaller.btnDrawInterlockClick(Sender: TObject);
var
...
procedure TfmCaller.btnFinishClick(Sender: TObject);
begin
  LeatherFinish := True;
end;
...
procedure TfmCaller.AddToKnivesUsed(var CurrentKnife: integer);
begin
  inc(CurrentKnife);
  SetLength(KnivesUsed, CurrentKnife);
...
procedure TfmCaller.DrawBitmap(Pattern: TPattern;
                               var imgKnf: TImage);
...
procedure TfmCaller.btnDebugClick(Sender: TObject);
begin
  fmDebugger.visible := True;
end;

procedure TfmCaller.btnDescribeClick(Sender: TObject);
var
  f: TextFile;
...
function TfmCaller.PackNo(No: integer): integer;
begin
  case No of
...
procedure TfmCaller.seDebuggerZoomChange(Sender: TObject);
begin
  fmDebugger.Zoom := seDebuggerZoom.value;
end;

procedure TfmCaller.qParametersAfterOpen(DataSet: TDataSet);
begin
...
procedure TfmCaller.rbGangsClick(Sender: TObject);
begin
...
procedure TfmCaller.rbNoneClick(Sender: TObject);
begin
...
procedure TfmCaller.btnDiagonalFreeAngleClick(Sender: TObject);
const
  Fudge = 0.1;
...
procedure TfmCaller.btnStepClick(Sender: TObject);
begin
  LeatherStep := True;
end;

procedure TfmCaller.tblKnivesCalcFields(DataSet: TDataSet);
var
  Prime, NonPrime: real;
...
procedure TfmCaller.DoCalc(var PrimeInterlockArea, NonPrimeInterlockArea: real);
var
  FirstInterlock, Complete: Boolean;
...
procedure TfmCaller.SetCutResultsColour;
var
  i: integer;
