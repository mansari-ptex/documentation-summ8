...
uses
  ...
  FDTablePlus, ComCtrls, Times_Summs7, Times_Summs8, CmnTypes, Buttons, XStringGrid, XStringGridPlus,
  ToolWin, ActnList, frxClass, Clipbrd, frxReportPlus, CutUtils_Summs7, CutUtils_Summs8;
...
begin
  if BusyPrinting or (Pos('Loading...', Caption) > 0) or Loading then
  begin
...
//  for i := 1 to qConParts.RecordCount do
  ...
  begin
    ...
    pbCalculating.Position := Round(i / LinesInGrid * 100);
...
//    sgSampleResults.Row := sgSampleResults.RowCount - 1;
//    sgCostedResults.Row := sgCostedResults.RowCount - 1;
...
//  sgSampleResults.Row := 1;
//  sgCostedResults.Row := 1;
...
function TfmConstructionAllowance.GetLoading: Boolean;
begin
  Result := FLoading;
end;
...
constructor TfmConstructionAllowance.Create(AOwner: TComponent);
begin
  inherited;
...
procedure TfmConstructionAllowance.SetLoading(const Value: Boolean);
begin
  FLoading := Value;
  ...
  if Value then
    pbCalculating.Position := 0;

  pnlCalculating.Visible := FLoading;
end;
