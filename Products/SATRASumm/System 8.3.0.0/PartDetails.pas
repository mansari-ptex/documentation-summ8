...
type
  TfmPartDetails = class(TForm)
    ...
    qKnifeSetsSizeScale: TStringField;
    qKnifeSetsType: TStringField;
    qKnifeSetsManualEntry: TBooleanField;
    qKnifeSetsCutgap: TSmallintField;
    qKnives: TFDQuery;
    ...
    procedure tblPartWidthKnifeBeforeRefresh(DataSet: TDataSet);
  private
...
begin
  ...
  else if not fmSumms.qSizeRelationships.Locate('Relationship', tblPartWidthKnife.FieldByName('SizeRelationship').Value, []) then
  begin
    if sMessage <> '' then
      sMessage := sMessage + ' and' + sLineBreak;
...
procedure TfmPartDetails.tblPartWidthKnifeBeforeRefresh(DataSet: TDataSet);
begin
  qKnifeSets.Refresh;
end;
...
begin
  ...
  qWidthsForRange.Refresh;  //TAH Refresh IS REQUIRED SO ADDED BACK IN
...
begin
  ...
  begin
    ...
    if not Failed then
    begin
      //CJY retaining cbWidthsForRange value despite refresh as described
...
procedure TfmPartDetails.LocalConnectionSummsAfterConnect(Sender: TObject);
begin
...
begin
  ...
  if qKnifeSets.FindKey([Knife]) and qKnifeSetsManualEntry.value then
  ...
  begin
    ...
    else if not qKnives.FindKey([Knife, Scale, AdjustedSize]) then
    ...
    begin
      if (qKnifeSetsCutGap.value > 0) and not(qKnifeSetsCutGap.value = Cutgap) then
        MessageDlgPos('The Cut gap on this Knife is ' + intToStr(qKnifeSetsCutGap.Value) +
