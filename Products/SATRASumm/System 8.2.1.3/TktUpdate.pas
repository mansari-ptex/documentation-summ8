...
procedure TfmTicketUpdate.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
begin
  ...
  tblTicketUpdateBulkUpdate.Value := false;
...
procedure TfmTicketUpdate.LocalConnectionSummsAfterConnect(Sender: TObject);
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      if (SelectedIndex < dbgTktUpdate.Columns.Count - 1) then
      begin
        i := SelectedIndex + 1;
        while (i < dbgTktUpdate.Columns.Count) and (not dbgTktUpdate.Columns[i].visible) do
          inc(i);

        if i > (dbgTktUpdate.Columns.Count - 1) then
          GotoNextRecord := True
        else
          SelectedIndex := i;
      end
