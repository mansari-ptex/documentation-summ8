  ...
  repeat
    ...
    begin
      ...
      begin
        ...
        //Synthronise tblTicketsequences to qTicketSequences
    ...
    try
      ...
      begin
        ...
        if fmCancelPrinting.Visible then
          fmfrTicket.qTickets.SQL.Text := 'SELECT TT.*, TS.Style, TS.StyleDescription, TS.Construction, TS.TagNo, TS.Customer, TS.Picture, TS.RowsInLeatherGrid, TS.LinesInLeatherGrid ' +
                                          'FROM TicketTickets TT, TicketSequences TS ' +
...
begin
  ...
  qExternalOutKnives.SQL.Text := 'SELECT * ' +
                                 'FROM TicketPairage TP ' +
                                 'WHERE (' + WeekSeqString('TP') + ') ' +
                                 'Order By WeekNo, WeekNo, SequenceNo, TicketNo, KnifeIndex, SizeIndex';

  qExternalOutTimes.SQL.Text := 'SELECT * ' +
                                'FROM TicketTicketsTimes TTT ' +
                                'WHERE (' + WeekSeqString('TTT') + ') ' +
  ...
  begin
    ...
    begin
      ...
      write(F, s);
      sFilter := sFilter + ' AND SequenceNo = ' + s;

      write(F, '/');
      str(qExternalOut.FieldByName('TicketNo').value : 4 : 0, s);
      write(F, s);
      sFilter := sFilter + ' AND TicketNo = ' + s;

      qExternalOutKnives.Filter := sFilter;
      qExternalOutTimes.Filter := sFilter;
...
procedure TfmAllTickets.LocalConnectionSummsAfterConnect(Sender: TObject);
begin
